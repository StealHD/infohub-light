#!/usr/bin/env python3
"""Explicit offline installer for global 33 ActorOps stability state."""
...
from src.storage.actorops_v2_stability_schema import (
    ...
    migration_marker_exists, partial_schema_present, prerequisite_ready,
    schema_shapes_valid,
)
...
def _state(connection: sqlite3.Connection) -> str:
    ...
    if row is not None:
        if (
            str(row["name"]) != MIGRATION_NAME
            or str(row["checksum"]) != MIGRATION_CHECKSUM
        ):
...
def migrate(
    ...
    if result["status"] == "blocked":
        raise RuntimeError("API and Worker must stop before global 33 migration")
    ...
    backup = raw_backup.with_name(raw_backup.name.replace(
        "service-apify-actor-ops-v15-", "service-actorops-v2-v33-", 1
    ))
