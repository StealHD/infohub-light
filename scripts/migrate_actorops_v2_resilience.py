#!/usr/bin/env python3
"""Explicit offline installer for global 31 ActorOps resilience facts."""
...
def migrate(data_dir: Path, *, apply: bool, backup_dir: Path | None = None) -> dict[str, Any]:
    ...
    backup = raw_backup.with_name(raw_backup.name.replace("service-apify-actor-ops-v15-", "service-actorops-v2-v31-", 1))
    ...
    try:
        ...
        if integrity.casefold() != "ok" or foreign_keys or not migration_marker_exists(connection) or not schema_shapes_valid(connection):
        ...
        raise
    return {"status": "applied", "required": False, "backup": str(backup), "backup_mode": "0o600", "integrity_check": "ok", "foreign_key_violations": 0, **changes}
