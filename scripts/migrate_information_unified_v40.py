#!/usr/bin/env python3
"""Explicit offline installer for global 40 unified automation."""
...
from src.storage.information_unified_schema import (
    TABLES,
    MIGRATION_CHECKSUM,
    MIGRATION_NAME,
    MIGRATION_VERSION,
...
def _state(connection: sqlite3.Connection) -> str:
    ...
    if row is not None:
        if str(row["name"]) != MIGRATION_NAME or str(row["checksum"]) != MIGRATION_CHECKSUM:
            raise RuntimeError("global schema migration version 40 is already occupied")
        if not schema_shapes_valid(connection):
            raise RuntimeError("unified automation marker exists with an invalid schema")
        return "ready"
    if not prerequisite_ready(connection):
        raise RuntimeError("valid global schema 39 is required before unified automation migration")
    tables = {
    ...
    if set(TABLES) & tables:
        raise RuntimeError("partial unified automation schema must be restored")
    return "required"
...
def preview(data_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        count = connection.execute("SELECT count(*) FROM information_rules WHERE state<>'archived'").fetchone()[0]
    finally:
        connection.close()
    ...
    return {
        ...
        "rules_to_upgrade": count if state != "ready" else 0,
...
def migrate(
    ...
    if result["status"] == "blocked":
        raise RuntimeError("API and Worker must stop before global 40 migration")
    ...
    backup = raw_backup.with_name(
        raw_backup.name.replace(
            "service-apify-actor-ops-v15-", "service-information-unified-v40-", 1
        )
    )
