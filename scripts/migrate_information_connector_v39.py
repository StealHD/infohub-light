#!/usr/bin/env python3
"""Explicit offline installer for global 39 information connector."""
...
from src.storage.information_connector_schema import (
    TABLES,
    MIGRATION_CHECKSUM,
    MIGRATION_NAME,
    MIGRATION_VERSION,
...
def _state(connection: sqlite3.Connection) -> str:
    ...
    if row is not None:
        if str(row["name"]) != MIGRATION_NAME or str(row["checksum"]) != MIGRATION_CHECKSUM:
            raise RuntimeError("global schema migration version 39 is already occupied")
        if not schema_shapes_valid(connection):
            raise RuntimeError("information connector marker exists with an invalid schema")
        return "ready"
    if not prerequisite_ready(connection):
        raise RuntimeError("valid global schema 38 is required before information connector migration")
    tables = {
    ...
    if set(TABLES) & tables:
        raise RuntimeError("partial information connector schema must be restored")
    return "required"
...
def migrate(
    ...
    if result["status"] == "blocked":
        raise RuntimeError("API and Worker must stop before global 39 migration")
    ...
    backup = raw_backup.with_name(
        raw_backup.name.replace(
            "service-apify-actor-ops-v15-", "service-information-connector-v39-", 1
        )
    )
