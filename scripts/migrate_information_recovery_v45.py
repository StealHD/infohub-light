...
from src.storage.information_recovery_schema import apply_migration, ready, prerequisite_ready
...
def migrate(data, apply=False):
    ...
    backup = original.with_name(original.name.replace('service-apify-actor-ops-v15-', 'service-information-recovery-v45-', 1))
    ...
    return {'status': 'applied', 'backup': str(backup), 'previews_confirmed': 0}
