#!/usr/bin/env python3
"""Explicit offline installer for global 37 personal Agent bindings."""
...
from src.storage.agent_connection_schema import (
    MIGRATION_CHECKSUM,
    MIGRATION_NAME,
    MIGRATION_VERSION,
    apply_migration,
...
def _state(connection: sqlite3.Connection) -> str:
    ...
    if row is not None:
        if str(row["name"]) != MIGRATION_NAME or str(row["checksum"]) != MIGRATION_CHECKSUM:
            raise RuntimeError("global schema migration version 37 is already occupied")
        if not schema_shapes_valid(connection):
            raise RuntimeError("agent connection marker exists with an invalid schema")
        return "ready"
    if not prerequisite_ready(connection):
        raise RuntimeError("valid global schema 36 is required before agent connection migration")
    tables = {
        str(item[0]) for item in connection.execute(
            "SELECT name FROM sqlite_master WHERE type='table'"
        )
    }
    if {"agent_connections"} & tables:
        raise RuntimeError("partial agent connection schema must be restored")
    return "required"
...
def migrate(
    ...
    if result["status"] == "blocked":
        raise RuntimeError("API and Worker must stop before global 37 migration")
    ...
    backup = raw_backup.with_name(
        raw_backup.name.replace(
            "service-apify-actor-ops-v15-", "service-agent-connections-v37-", 1
        )
    )
