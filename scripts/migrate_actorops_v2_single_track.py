#!/usr/bin/env python3
"""Explicit offline installer for global 30 ActorOps v2 single-track schema."""
...
def _shadow_count(connection: sqlite3.Connection) -> int:
...
def preview(data_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        shadows = _shadow_count(connection) if state == "required" else 0
    finally:
        connection.close()
...
def migrate(
    ...
    if result["status"] == "blocked":
        raise RuntimeError("API and Worker must stop before global 30 migration")
    ...
    backup = raw_backup.with_name(
        raw_backup.name.replace(
            "service-apify-actor-ops-v15-", "service-actorops-v2-v30-", 1
        )
    )
