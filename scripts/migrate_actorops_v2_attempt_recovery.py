#!/usr/bin/env python3
"""Explicit offline installer for global 29 ActorOps Attempt recovery."""
...
from src.storage.actorops_v2_attempt_recovery_schema_sql import REQUIRED_COLUMNS
...
def _state(connection: sqlite3.Connection) -> str:
    ...
    if row is not None:
        ...
        if not schema_shapes_valid(connection):
            raise RuntimeError("Attempt recovery marker exists with an invalid schema")
        return "ready"
    columns = {
        str(info[1])
        for info in connection.execute("PRAGMA table_info(actor_attempts_v2)")
    }
    if columns & REQUIRED_COLUMNS:
        raise RuntimeError("partial Attempt recovery schema must be restored")
    if not prerequisite_ready(connection):
        raise RuntimeError("valid global schema 28 is required before Attempt recovery")
    return "required"
...
def preview(data_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        legacy_count = (
            int(connection.execute("SELECT COUNT(*) FROM actor_attempts_v2").fetchone()[0])
            if state == "required" else 0
        )
    finally:
        connection.close()
...
def migrate(
    ...
    if result["status"] == "blocked":
        raise RuntimeError("API and Worker must stop before global 29 migration")
    ...
    backup = raw_backup.with_name(
        raw_backup.name.replace(
            "service-apify-actor-ops-v15-", "service-actorops-v2-v29-", 1
        )
    )
    ...
    return {
        ...
        "backup": str(backup),
        "backup_mode": "0o600",
        "legacy_attempt_count": result["legacy_attempt_count"],
        "integrity_check": "ok",
