#!/usr/bin/env python3
"""Explicit offline installer for global 38 information reminders."""
...
from src.storage.information_automation_schema import (
    TABLES,
    MIGRATION_CHECKSUM,
    MIGRATION_NAME,
    MIGRATION_VERSION,
...
def _state(connection: sqlite3.Connection) -> str:
    ...
    if row is not None:
        if str(row["name"]) != MIGRATION_NAME or str(row["checksum"]) != MIGRATION_CHECKSUM:
            raise RuntimeError("global schema migration version 38 is already occupied")
        if not schema_shapes_valid(connection):
            raise RuntimeError("information automation marker exists with an invalid schema")
        return "ready"
    if not prerequisite_ready(connection):
        raise RuntimeError("valid global schema 37 is required before information automation migration")
    tables = {
    ...
    if set(TABLES) & tables:
        raise RuntimeError("partial information automation schema must be restored")
    return "required"
...
def migrate(
    ...
    if result["status"] == "blocked":
        raise RuntimeError("API and Worker must stop before global 38 migration")
    ...
    backup = raw_backup.with_name(
        raw_backup.name.replace(
            "service-apify-actor-ops-v15-", "service-information-automations-v38-", 1
        )
    )
