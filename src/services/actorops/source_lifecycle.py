"""Shared v2-managed source lifecycle for HTTP and Agent mutations."""
...
def is_actorops_managed_source(source_type: str, config: object) -> bool:
...
def agent_source_update_requires_web(
    source: Mapping[str, Any], updates: Mapping[str, Any] | None
) -> bool:
    return bool(
        updates
        and {"config", "enabled"}.intersection(updates)
        and source.get("type") == "apify_social"
        and is_actorops_managed_source(str(source.get("type")), source.get("config"))
    )
...
class ActorOpsSourceLifecycle:
    ...
    @staticmethod
    def is_managed(source_type: str, config: object) -> bool:
        return is_actorops_managed_source(source_type, config)
...
__all__ = [
    ...
    "agent_source_update_requires_web",
    "assert_actorops_subscription_enable_allowed",
    "is_actorops_managed_source",
]
