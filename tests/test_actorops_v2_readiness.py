...
from src.storage.actorops_v2_attempt_recovery_schema_sql import REQUIRED_TRIGGERS
...
def test_disabled_flag_never_queries_global_26(tmp_path, monkeypatch) -> None:
    ...
    assert not any(
        ...
        or "version=29" in statement.replace(" ", "")
...
def test_enabled_flag_requires_complete_global_29(tmp_path, monkeypatch) -> None:
    store = ServiceStore(tmp_path / "data")
    store.initialize()
    monkeypatch.setenv("ACTOROPS_V2_ENABLED", "true")
    connection = store.connect()
    connection.execute(f"DROP TRIGGER {next(iter(REQUIRED_TRIGGERS))}")
    connection.commit()
    ...
    try:
        ...
        raise AssertionError("partial global 29 must fail closed")
    store.close()
