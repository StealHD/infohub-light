...
def _seed_manual_refresh_subscription(data_dir, *, username="owner", scope="private"):
...
def test_user_feed_refresh_job_endpoint_creates_queued_job(tmp_path, monkeypatch):
    client, data_dir = _client(tmp_path, monkeypatch)
    _login(client)
    _seed_manual_refresh_subscription(data_dir)

    response = client.post(
        "/api/jobs/user-feed-refresh",
        json={"payload": {"reason": "manual", "refresh_scope": "private"}},
    )
...
def test_manual_refresh_scope_is_server_owned_by_current_role(
    ...
    _seed_manual_refresh_subscription(data_dir, username=role, scope="private")
    store = ServiceStore(data_dir)
    store.initialize()
    workspace = store.get_default_workspace()
    ...
    client.post("/api/auth/logout")
    _login_as(client, role, f"{role}-password")
    ...
    response = client.post(
        ...
        json={
            "payload": {
                "reason": "forged",
                "refresh_scope": "all" if role == "member" else "private",
            }
        },
    )
...
def test_running_feed_refresh_cancel_is_persistent_idempotent_and_summary_visible(
    ...
    first = client.post(f"/api/jobs/{created['id']}/cancel")
    second = client.post(f"/api/jobs/{created['id']}/cancel")
