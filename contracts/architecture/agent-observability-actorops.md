...
`src/mcp/remote_server.py` 是现有 FastAPI 上的无状态 Streamable HTTP adapter；20 个工具由四类单向 registrar 组合：13 个有界读/引导/发现/诊断工具、4 个订阅 proposal/apply 工具和 3 个系统参数工具。`remote_tool_registration.py` 只组合 registrar，业务仍直接调用专用 Service/Store；任何工具都禁止内部 HTTP 回环、开放式网页搜索或调用未经 adapter 校验的任意 URL。每个 FastAPI app 拥有独立 FastMCP 和 session manager，父 app lifespan 显式管理其生命周期，`/mcp` 与 `/api/*` 共用请求级 SQLite connection scope 和事务泄漏检查。
...
Phase 5 已实现本边界中的 Domain、Port、Registry、Policy、分拆 Repository、三类 Adapter、Runtime、薄 Service、条件 feature flag、global 26、只读 Reconciler、Worker claim 隔离、可恢复 Discovery 与受限站立维护。Phase 6 只增加离线 Route CAS、脱敏切流摘要/backup CLI、shadow 选择事件与稳定身份 bridge。`catalog_binding_bridge.py` 是独立的离线 source-catalog→v2 binding bridge：它以 RouteKey/Registry/Adapter 重验既有 `apify_social` target，只能为缺少任何 v1 binding 的来源创建 pending v2 binding，不写目录来源、旧 binding、LKG、水位、Attempt 或 Route mode；`repair_actorops_v2_catalog_bindings.py` 只组合其 dry-run、停止服务检查、`0600` backup/恢复与 SQLite postcheck。`legacy_readiness.py` 只从现役 v1 slot、revision 与 settled source-canary 事实导出可提升的 existing binding：它忽略终态或不完整 slot，要求当前 runnable exact revision 顺序与 v2 assignment 完全相等，且不读取 Secret、Job、Feed 或平台代码；`repository_bindings.py` 只在 caller transaction 内以 status/version/fingerprint CAS 写 v2 binding。`repair_actorops_v2_binding_readiness.py` 是这两个小模块的离线组合根，不能将 catalog pending bridge 解释为 ready，也不能改变 v1 Candidate pool。`route_controls.py` 只重排同一路线中既有 runnable Active/Standby Candidate，所有中间 assignment 与 Route generation 都在同一 caller transaction 内 CAS；`actorops_v2_control_routes.py` 是 Owner/Admin HTTP adapter，它只允许该无费用重排和 `legacy_readiness` 已证明 binding 的 pending→ready 提升，并先经过 flag/readiness；它不能访问 target、Secret、Job、Feed、Manifest、Actor remote client 或任何平台 Adapter。React 的 `ActorOpsV2RouteControls` 只调用这两个脱敏 action，`ActorOpsV2ControlPlane` 仍只消费投影，二者不能反向访问 SQL、Job、Secret、Feed 或旧 Batch/Stage shape。legacy 历史费用隔离是独立离线边界：`legacy_cost_audit.py` 只分类事实、`legacy_cost_evidence.py` 只管理加盐 opaque resume session、`legacy_cost_mutations.py` 只执行 CAS；`audit_actorops_v2_legacy_costs.py` 是唯一允许持有 legacy Apify lease 并执行单 Run authenticated GET 的组合 Adapter。它只在初次/明确 resume 页面访问远端，snapshot 只验证完整 session 并创建私有 backup/receipt，禁止 POST、abort、Dataset/来源读取。global 26 migration 仍只读旧事实：只有一个已知 remote Run 且该 Run 本身保存 terminal/final actual cost 时，才可作为其已终态 Attempt 遗失结算派生位的精确证明；多个 Run、空实际费用或任何非终态永不放行。Phase 7 的 `actorops_v2_projection.py` 是 v1 admin URL 的 additive facade，`actorops_v2_maintenance_routes.py` 仅以 policy generation CAS 改变默认关闭的授权；这些路径都先检查 flag/readiness，flag=false 时完全不读 global 26。Discovery 只使用 Catalog Port 的 Store/Actor/Build 元数据读取；每个 checkpoint 仅保留有界安全引用和 hash，具体平台 Schema→Manifest 映射留在订阅类型 Adapter，AI 输出必须再次通过 exact Schema。站立维护以 Owner/Admin 双 policy 授权、免费 exact-revision 预检、单 Candidate Probe 和独立 Attempt 账本实现；平台切流仍为 planned。全部 Route 默认 disabled，因此第 3.6J 的现役 v1 所有权不变。
...
2. `repository.py` 是唯一公开 Repository；其 focused `repository_*` 内部模块分担 read、Candidate、Attempt、Discovery、publication 与 operator-control SQL，避免单文件增长。`repository_operator.py` 只保存安全 Store 元数据、Route cap CAS 与 Replacement Plan 的冻结/应用；`store_metadata.py` 仅归一公开 allowlist。其他通用模块和 Adapter 不得直接导入 SQLite、旧 `ServiceStore` 私有方法或表名。`policy.py` 只根据领域快照计算候选顺序、熔断、健康、补池和预算决定，不执行网络或写入。
   global 26 的建表、只读 v1 摘要 backfill 与离线 migration/目录 binding repair 是 storage 例外，分别归 `src/storage/actorops_v2_*`、`scripts/migrate_actorops_v2.py` 与 `scripts/repair_actorops_v2_catalog_bindings.py`；D173 的 v1 cap migration 仅归 `src/storage/apify_actor_validation_cap_v27_schema.py` 和 `scripts/migrate_apify_actor_validation_cap_v27.py`。这些离线模块不得成为在线 Repository 或 Runtime 依赖；在线 pool setter 只以 marker 判定是否允许超过旧 `$0.10`。
3. `runtime.py` 只编排 Active、Standby、Last Known Good 与发布凭据；Apify remote wrapper 把 unknown-start 保留在单 Attempt/reservation，禁止调用 v1 workspace barrier。`reconciliation.py` 只依赖 v2 Repository 和 RemoteRunLedger Port；`apify_ledger.py` 是唯一允许按 `logical_run_id=attempt_id` 读取/结算 `apify_actor_runs` 的组合 Adapter。`replacement.py` 是不发布数据的通用串行 Probe 编排：一项计划一次只允许一个 frozen Binding/Attempt，预检、输出合同与最终费用都通过才继续；`worker_actorops_v2_metadata.py` 和 `worker_actorops_v2_replacement.py` 仅组合 flag/readiness、公开 Catalog、Remote 与队列边界。Worker 先完成轻量 queue claim，已完成 Job 后或 idle 时才运行有界、隔离异常的 `worker_housekeeping.py`；Provider/Reconciler/metadata/replacement 失败不得阻塞普通 Job。`discovery.py` 只推进可恢复 checkpoint；`maintenance.py` 不知道平台或 Feed，只在双授权、免费 Catalog preflight 和原子预算 reservation 已通过后执行单 Candidate Probe，再调用 Repository 的补位/非最后一路替换；它不恢复 Dataset、不触发 fallback 或改写 LKG/水位。`worker_actorops_v2_maintenance.py` 只负责 flag/readiness、低优先级 Job 边界与依赖组合；`service.py` 为 Worker 和 source acquisition 提供薄 facade。
4. 每个 `platform + target_type + capability` 使用独立 Adapter，例如 `youtube/channel/items` 与未来 `youtube/video/comments` 必须是两个 Adapter，可复用 `youtube/common.py` 的身份规范化。Adapter 实现上述端口而不继承通用工作流，只负责目标规范化、Store 查询/静态能力描述、Actor input 构造、输出身份/内容验证、`ContentItem` 映射和可选原生降级；X profile 的帖子关系证据与回复排除也只归其平台 Adapter/helper，通用 Manifest、Runtime、Feed 和前端不得解析 X 原始字段。Adapter 不得选择 Candidate、管理 Key/预算、写状态、创建 Job、发布 Feed 或处理重启恢复。
