...
1. API / event 入口层：`src/api/server.py` 作为 composition root，`src/api/context.py` 提供 typed request-independent 依赖，system/auth、member administration、Feed/read-media、Feed 与来源 schedule、用户订阅与 source-health、storage governance、queued-job、Agent delegation、通知服务/目标/个人设置、工作区通知 transport、Actor 告警设置/事件、Apify Key Pool、SecretStore、Catalog 只读元数据与来源成员关系 HTTP 适配分别归 `src/api/system_auth.py`、`src/api/user_routes.py`、`src/api/feed_routes.py`、`src/api/schedule_routes.py`、`src/api/subscription_routes.py`、`src/api/storage_routes.py`、`src/api/job_routes.py`、`src/api/agent_delegation_routes.py`、`src/api/notification_routes.py`、`src/api/notification_transport_routes.py`、`src/api/actor_alert_routes.py`、`src/api/apify_key_pool_routes.py`、`src/api/secret_routes.py`、`src/api/catalog_metadata_routes.py`、`src/api/catalog_membership_routes.py`；ActorOps 只读 route/pool/freshness/event/Canary plan 与 batch HTTP 适配归 `src/api/actor_ops_read_routes.py`，路由、revision、Canary plan 与 batch 的安全公共投影归 `src/api/actor_ops_projection.py`，两者不得包含 SQL、远端调用、审批或费用写入。Worker 入口保留在 `src/services/worker.py`，claim 前周期编排、Actor 周期维护与 validation/freshness/discovery/Canary handler、来源/Feed handler registry、lifecycle/media publication、claim-token 事务终结和提交后分发/终态遥测分别归 `src/services/worker_cycle.py`、`src/services/worker_actor_cycle.py`、`src/services/worker_actor_validation_handler.py`、`src/services/worker_actor_discovery_handler.py`、`src/services/worker_actor_discovery_ai.py`、`src/services/worker_actor_canary_handler.py`、`src/services/worker_handlers.py`、`src/services/worker_feed_handler.py`、`src/services/worker_lifecycle.py`、`src/services/worker_media_publication.py`、`src/services/worker_finalization.py` 与 `src/services/worker_post_commit.py`，Remote MCP HTTP 与 Worker job handler 同属入口层。这里只负责参数接收、认证、校验和薄编排，领域服务与事务语义不得搬进入口模块。
当前 ActorOps Worker 单轨边界取代上项中旧 Actor handler 的现役归属：`worker.py`、`worker_cycle.py`、`worker_housekeeping.py`、`worker_job_policy.py` 与 `worker_retired_actorops_jobs.py` 只允许四种 v2 ActorOps Job 进入 claim、stale lease recovery 与执行；普通获取 Job 不依赖任一 v1 ActorOps migration。未开始的四种 v1 Job 只可在 `fetch_jobs` 原子标记为 `actorops_v1_retired`，已 claim、运行或费用不明的 v1 事实留给离线退役工具；历史 handler 仅供 migration、审计、retirement 或指定历史测试使用。

2. Service 层：`src/orchestrator.py`, `src/services/**`。负责抓取、去重、可选分析、用户 Feed finalization、读取与留存；配置运行时、来源探测、Agent 来源规范化、订阅事务与 ActorOps 来源 Binding 生命周期、Feed payload/read 都归中性 Service 模块。
