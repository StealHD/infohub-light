...
This directory, indexed by this file, is the sole source of truth for production UI technology, visual language, interaction behavior, component parameters, responsive behavior, and browser acceptance. Cross-route interaction stability is owned by `interaction-constitution.md`; component numbers remain owned by `component-parameters.md`. `PLAN.md` records delivery state, `docs/decisions/` records durable choices, and `tests/test_impact_map.json` selects verification; they must reference this contract rather than restate its visual rules. API fields, authorization, query ownership, and error envelopes remain governed by `docs/contracts/api/` and existing application tests.
