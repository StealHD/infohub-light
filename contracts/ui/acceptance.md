...
- Automations 编辑补验长描述四行展开、来源前二摘要、来源搜索分页与确认/取消、平台名称去重、直接可见的模型选择、sticky 保存、未保存提示与独立启用确认。已归档任务补验单次恢复为未确认草稿，并确认不会自动启用、补跑或发送。测试补验默认不选不执行、最新 Feed 来源过滤、每页 10 条、选择快照、重复点击、账号/任务/版本隔离、后台轮询与列表状态、旧版本结果、失败和提交未知的手动恢复、无取消操作及“不发送通知”。运行记录默认紧凑并按次展开。覆盖明暗、Reduced Motion、200% 缩放和 Axe；浏览器批次必须正常退出并清理临时服务。
