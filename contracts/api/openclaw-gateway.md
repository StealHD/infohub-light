...
- 服务端托管连接在握手的 `features.methods` 中协商本地 `relay.ping`。该方法仅接受空对象、只校验当前登录与绑定并返回 `{alive:true}`；它不转发 Gateway、不调用模型、不读写会话，也不增加 scope。
- 浏览器仅在页面可见且网络在线时每 20 秒保持最多一个探测请求；30 秒无响应按瞬时断线处理。页面恢复可见或网络恢复时立即探测，后台计时器延迟本身不得判定连接失败。
- 瞬时断线从约 1 秒开始指数退避并带随机抖动，最大 30 秒；连续稳定 60 秒后重置。任一页面同时只能有一个连接尝试和一个重连定时器。登录失效、账号停用或个人绑定撤销使用 WebSocket 1008 并停止自动重连，其他上游连接失败继续恢复。
- 重连期间保留会话、历史、草稿和未确认运行；恢复后读取原会话核对状态，不自动重发聊天、切换模型或执行其他业务写入。原会话明确不存在时停止恢复并要求用户处理，不静默创建替代会话。
- 每条已接受的 Relay 连接生成 `req_<uuid>`，通过 WebSocket 握手 `X-Request-ID` 返回，并在安全 runtime 日志中记录持续时间、失败阶段、浏览器/上游关闭码与上游握手状态。日志不得包含 Cookie、凭据、URL、会话 key、正文或原始异常；正常 1000/1001 关闭只记 info。
