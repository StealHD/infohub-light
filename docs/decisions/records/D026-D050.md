...
- 兼容收敛（2026-09-01）：VPS 旧数据可能没有可安全复用的 `source_native_title` 用户供体，但共享获取池仍保留规范中性条目；新订阅因此在用户稳定索引之后回退到同 workspace 中性缓存，并在托管来源本地启用对账前完成目标订阅投影。回退最多 200 条、零网络、零 AI，不读取 private isolation 或继承供体用户状态。
