...
- 费用与证据边界：快速上线不改变 Route `$0.02` 默认单次上限，不把 static-valid、超时或合同失败候选伪装成 runnable，也不自动触发 AI、Canary 或 Actor。D136 已取代本决策中“已有两个安全 Actor 后永久隐藏完整 2+1 付费入口”的限制，D137 又将补位准入扩为已验证可运行的 probationary/certified exact 两路：管理员可在原池不中断的前提下手选并预验证第三槽。
...
- 决策日期：2026-08-04
- 当前状态：原生路由已经成立；D136 进一步替换本决策的高密度 Route 表与“只组合旧 API”边界
...
- 交互与按需读取：D136 取代本段的高密度 Route 表，改为一个固定抓取类型选择器、三个任务页签和服务端权威的唯一下一步。Revision 历史/回滚、单次费用上限、候选明细和 Discovery AI 仍默认折叠，并仅在对应 disclosure 打开时读取；告警改为运行页签中的摘要与编辑 Modal，事件默认只展开最新五条。
