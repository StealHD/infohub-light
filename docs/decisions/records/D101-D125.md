...
- 费用与证据边界：快速上线不改变 Route `$0.02` 默认单次上限，不把 static-valid、超时或合同失败候选伪装成 runnable，也不自动触发 AI、Canary 或 Actor。D136 已取代本决策中“已有两个安全 Actor 后永久隐藏完整 2+1 付费入口”的限制：两路自动认证后可以在原池不中断的前提下，用 Pool Stage 预验证并补齐第三槽。
