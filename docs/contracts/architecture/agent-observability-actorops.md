...
`ActorOpsAdminService` 是现役管理读模型的唯一组合服务：它只经 v2 Repository 查询 Route、Candidate、Binding、Attempt、Discovery、Maintenance、Replacement 与安全 Store metadata，既不构造 `ApifyActorOpsService`，也不读取任何 v1 Route、Pool、Canary、Freshness 或 diagnostic 表。`actorops_admin_routes.py` 只负责 Owner/Admin 鉴权、HTTP envelope 与 503 映射；list/detail 固定为 `schema_version=2`，并对 target fingerprint、水位、Manifest、remote Run/Dataset、idempotency 与 Secret 相关字段脱敏。ActorOps 事件只由脱敏的 `OperationLogQueryService` 提供，且只暴露 `actorops_v2_*` action。v2 Admin 不由 `ACTOROPS_V2_ENABLED` 决定语义；缺少所需 v2 schema 为 `actorops_v2_migration_required`，其他存储不可用为 `actorops_v2_unavailable`。旧 Pool/Canary/Freshness HTTP 适配器仍在其退役阶段内使用局部 legacy composition，不能反向成为 `ApiContext` 或任何 v2 Admin 路径的依赖。
