...
ActorOps v2 global 30 由 `scripts/migrate_actorops_v2_single_track.py` 独占安装：它只在 global 29 完整、API/Worker 已停止且 heartbeat 安全窗已过时允许 `--apply`，先创建 `0600` backup，再重建当前 v2 Route/Binding 表并写最终 `actorops_v2_single_track` marker。该表层只保留 `active|disabled`，历史 `shadow` 离线归一为 disabled，`source_v1_generation` 从当前 v2 状态删除；route/binding trigger、marker/shape、integrity 与 foreign keys 任一失败均恢复完整 backup。fresh `ServiceStore` 通过 Adapter Registry 直接种入 disabled Route 和 disabled Maintenance Policy，不执行 v1 backfill 或构造 v1 runtime。该 migration 只影响 ActorOps：marker/shape 缺失时 ActorOps API/Job fail closed 为 migration-required，普通 RSS/GitHub、source fetch 与 user-feed refresh 继续运行；它不联网、不调用 Actor/AI、不删除 v1 表或改写费用。

`src/services/actorops/legacy_aggregate_costs.py` 与 `scripts/finalize_actorops_v1_historical_costs.py` 是 global 30 之前唯一允许补齐**终态历史费用派生字段**的离线组合：它只读取 v1 Aggregate 与共享 `apify_actor_runs` ledger，并把 `scan` 的加盐 opaque fact、状态/版本、精确金额和 hash 作为 `0600` evidence。必须停 API/Worker、跨 heartbeat 安全窗、先 snapshot/backup/receipt，再由 hash 与当前行 CAS apply；候选 Attempt 仅限一个 terminal/final Run，缺 remote ID 时仅限 `start_rejected`、无 Dataset 且零预留/零实际。Validation、Batch item、Batch 只能沿这条已最终的金额链或终态无 Attempt cancelled/failed 的零费用链汇总；如果完整 settled Batch 子项低于其已有非空总额，只保留（不下调）该历史总额。它只从 NULL 补出派生金额并把 `cost_final` 置 1，永不覆盖非 NULL 金额、改动 Run、重开事实、联网或放行多 Run、金额低于已结算子项、unknown-start、非终态或其他未结费用。
...
Personal Agent global 37 使用 `scripts/migrate_agent_connections_v37.py --data-dir ABSOLUTE_PATH [--apply]`，以有效 global 36 为前置。既有库不自动升级；API/Worker 停止并跨过 heartbeat 窗口后创建 0600 SQLite backup，再事务安装空绑定表并通过 integrity/foreign-key/schema 检查，失败恢复备份。不迁移旧 main 会话、不创建默认绑定、不修改旧 delegation scope。缺迁移只阻断个人绑定与服务端 relay，不影响现役 Feed/Worker。首库引导直接安装空表。
...
本地 `up-latest` 与正式 VPS cutover 只能通过 `scripts/runtime_health.py` 判定运行面完成：目标版本/revision、API ready、Worker ready、API/Worker Docker health、两容器 source digest、React asset 与适用的公网 revision 必须同时成立。Docker health 为 `starting` 时继续有界等待，只有 `unhealthy` 或超时失败。回滚在旧 API/Worker 重新 healthy 前不得更新 `current` 或报告成功。需要恢复旧 schema 的非兼容迁移发布，必须将 `INTELISCOPE_PRE_MIGRATION_BACKUP` 指向 `$base/data/backups/` 下的非符号链接迁移前副本并在旧代码启动前恢复、校验；仅增加结构且旧运行时已在新 schema 上通过健康检查的 global 47 是明确例外，其回滚保留数据库以免丢失迁移后的写入。
