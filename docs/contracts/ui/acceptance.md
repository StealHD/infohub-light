...
- Any OpenClaw Workspace change verifies `/agent`, `/agent/tasks`, `/agent/artifacts`, `/agent/skills`, and `/agent/automations`, and `/agent/examples` at 1440×900, 1024×768, and 390×844 plus 200% zoom. It proves product switching restores the latest allowlisted per-user route, ordinary socket/session/run/draft survive Inscope ↔ OpenClaw and Feed → `/agent`, the shared 232 px sidebar and on-demand 360 px inspector follow the Drawer/Sheet contract, Tasks/Artifacts do not unmount conversation, current-run session switching is blocked, every unsupported/forbidden/failure state remains local, and no route has horizontal overflow. Admin checks prove authorization appears only on write intent, successful authorization does not replay the write, exact temporary scope, zero persistent secret/chunk/response writes, explicit trust-domain and write confirmation, close/unmount/idle cleanup, and isolation from ordinary chat events. Artifact checks cover preview/download limits, same-origin expiry, UTF-8 failure, and object-URL revoke; Axe has zero serious/critical findings in both themes and Reduced Motion.
- Managed Skills acceptance starts with an empty global policy, proves Owner/Admin can search/filter/check and confirm a revisioned update, and proves Member/Viewer cannot request the full catalog or mutate it. Skills page, details, `@`, and `/skills` expose exactly the same allowed keys; unavailable allowed rows explain public conditions and cannot be selected. Revocation invalidates stale detail/selection/account responses and blocks the next invocation while an already-running conversation can finish. Pending/failed synchronization preserves the intended checklist, blocks new chat, offers retry, and never renders success or all-open fallback. Repeat the flow at 1440×900, 1024×768, 390×844, dark/light, keyboard-only and Reduced Motion, with no horizontal overflow.
...
- Agent directory acceptance covers more than three rows, pages beyond 200, search/archive changes, late old-user/Gateway responses, cross-Agent exact-identity verification and failed switches preserving the original conversation. Title events update in place; opening history produces no create/send/rename RPC.
- Effort picker acceptance covers keyboard and pointer commits, reset to the Gateway-supported default, absence of off/auto choices and no implicit mutation for unknown defaults, model-specific/unsupported levels, one in-flight mutation, failure rollback, draft preservation, nested model selection, Escape/focus return, both themes and touch bounds; Ultra floats particles and Fast streams them, with Fast taking precedence when both are enabled; Fast activation and successful Ultra entry each show their own transient usage reminder, while failed changes show none; changing effort never toggles Fast. Verify true/false on the next send, original retry choices, session/user/Gateway isolation, hover and drag scale, borderless thumb, mark/thumb alignment at every step including both end-cap centers, unclipped model text, aligned full-width connection actions at narrow and wide widths, visible toolbar model/effort/context grouped before Send, active-theme accent in both themes, independent touch targets, automatic settling, and static Reduced Motion.
- UI-INT-06 acceptance checks opening, closing, immediate reversal, focus return, preserved center DOM/draft/scroll and Reduced Motion. Workspace Composer checks compact empty geometry, multiline input, long model labels, IME, attachments and keyboard shortcuts; account actions match Inscope.
...
- Automations 验收覆盖统一任务描述、四种触发、可选最长等待、每天/每周、模型目录不可用与刷新、只读和空状态；保存/刷新保持编辑 DOM 和草稿，双击只接收一次请求。大批量预览展示阶段进度及综合结论，测试不发送；真实通知以回执为准。三视口、明暗、Reduced Motion、键盘及 Axe 由 production-information-reminders.spec.ts 负责。
- Automations 列表补验状态/搜索与分页范围说明；详情补验 400 px 默认、320–720 px 与 640 px 列表边界、账号隔离、指针/方向键/Shift/Home/End/双击调宽，以及 1024/390 px Drawer/Sheet。拖动、筛选、刷新和概览/测试/运行记录切换保持编辑 DOM、草稿与滚动位置；pending 不关闭，关闭恢复任务或搜索焦点。
- Automations 编辑补验长描述四行展开、来源前二摘要、来源搜索分页与确认/取消、平台名称去重、模型标签与刷新入口同排、sticky 保存及未保存提示；不得显示模型读取原理、正常加载数量或逐项过滤原因。完整草稿／暂停任务在列表与详情单击“启动”，已启用任务单击“暂停”，重复点击只接收一次请求；启动不弹二次确认、不测试、不补跑历史，失败保留状态及恢复提示。已归档任务补验单次恢复为未确认草稿，并确认不会自动启用、补跑或发送。测试补验默认不选不执行、最新 Feed 来源过滤、每页 10 条、选择快照、重复点击、账号/任务/版本隔离、后台轮询与列表状态、旧版本结果、失败和提交未知的手动恢复、无取消操作及“不发送通知”。概览显示按时间倒序的最近三次真实运行，通知 sent 且没有回执仍显示未验证；完整运行记录默认紧凑并按次展开。会话行补验悬停／键盘焦点显示 X、触屏常显、Tooltip、禁用原因、删除确认和点击 X 不切换会话。覆盖明暗、Reduced Motion、200% 缩放和 Axe；浏览器批次必须正常退出并清理临时服务。
- Automations 列表启停使用固定尺寸图标与悬停／聚焦解释；请求中和完成后图标轨道、任务行及原有顺序不跳动。草稿、暂停、启用、归档任务均可由列表和详情的独立删除入口打开确认；取消不写入，确认后只发一次带版本删除请求，目录移除且后续处理停止，既有回执不被清理。验证只读、未保存、版本冲突、重复点击和失败恢复；模拟 API，不删除真实任务。
