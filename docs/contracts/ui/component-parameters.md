...
- Accessibility, responsive behavior and the cross-route laws in `interaction-constitution.md` always remain in force. A reference screenshot cannot remove accessible names, stable async geometry, focus behavior, Reduced Motion or supported viewports.
...
| Async action | `StableAsyncButton` | Satisfy UI-INT-01 through UI-INT-03. It synchronously locks the accepted activation before feature pending state renders, keeps the lock through returned Promise and external pending, and releases after success, failure or synchronous throw. A pending, queued, saving, testing, applying or other non-terminal state keeps the initiating button's outer width and height unchanged while exposing its current accessible name and busy state. Idle and intermediate content share one layout track; feature code must not replace a text button's children in a way that changes geometry. A genuine semantic mode switch may use different content only when an existing fixed-size role owns the geometry, or when a route contract records the exception and a browser test proves stable surrounding layout. |
| Refresh/retry action | `RefreshButton` / `RefreshIconButton` | Satisfy UI-INT-02 and UI-INT-04. `RefreshIconButton` owns the nearby Tooltip for compact icon-only refreshes. A user-triggered refresh or retry keeps its label, icon position and outer geometry stable. Press starts `RefreshCw` rotation immediately, disables repeated activation, exposes the pending accessible name and keeps feedback visible for at least 400 ms even when the request finishes faster; a longer request rotates until the returned Promise and external pending state have both completed. Reduced Motion preserves the busy/disabled state but removes rotation. Feature code must not bind `refetch()` to a plain `Button`. |
...
| Icon-only action | `TooltipTriggerButton` or design-system icon-only `Button` | 32 px compact fine-pointer target, 15 px icon, complete accessible name and Tooltip. The design-system `Button` and `TooltipTriggerButton` wrappers automatically enforce at least 44×44 px under coarse pointers without changing desktop geometry; other custom triggers must apply the same target explicitly. Header toggles may use the existing 34×32 fine-pointer geometry. |
| Truncated essential value | `OverflowValue` | Render ordinary clamped text when it fits. Only when real overflow exists, expose a keyboard, pointer and touch operable Popover trigger with a complete accessible name; the full value uses anywhere wrapping, Escape/external close restores focus, and HTML `title` or hover-only Tooltip is not the complete-value path. |
