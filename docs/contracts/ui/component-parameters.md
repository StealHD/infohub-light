...
| Refresh/retry action | `RefreshButton` | Satisfy UI-INT-02 and UI-INT-04. A user-triggered refresh or retry keeps its label, icon position and outer geometry stable. Press starts `RefreshCw` rotation immediately, disables repeated activation, exposes the pending accessible name and keeps feedback visible for at least 400 ms even when the request finishes faster; a longer request rotates until completion. Reduced Motion preserves the busy/disabled state but removes rotation. Feature code must not bind `refetch()` to a plain `Button`. |
...
| Icon-only action | `TooltipTriggerButton` or design-system icon-only `Button` | 32 px compact fine-pointer target, 15 px icon, complete accessible name and Tooltip. The design-system `Button` wrapper automatically enforces at least 44×44 px under coarse pointers without changing desktop geometry; non-Button custom triggers must apply the same target explicitly. Header toggles may use the existing 34×32 fine-pointer geometry. |
