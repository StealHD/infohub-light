...
| Async action | `StableAsyncButton` | Satisfy UI-INT-01 through UI-INT-03. A pending, queued, saving, testing, applying or other non-terminal state keeps the initiating button's outer width and height unchanged while exposing its current accessible name and busy state. Idle and intermediate content share one layout track; feature code must not replace a text button's children in a way that changes geometry. A genuine semantic mode switch may use different content only when an existing fixed-size role owns the geometry, or when a route contract records the exception and a browser test proves stable surrounding layout. |
