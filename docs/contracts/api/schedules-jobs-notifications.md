...
## Telegram 话题与 OpenClaw 自动化服务（global 47）

Telegram shared service create 接受可选 `telegram_message_thread_id`；省略或 null 使用默认话题。PATCH 省略保留原话题，null 清除，正整数替换；其它类型、零和负数拒绝。公开投影只增加 `telegram_topic_configured`，不公开 ID。话题更改推进配置代次并使旧测试和未开始投递失效。同一 Chat ID 可通过多个服务分别绑定话题；发送 API 不自动创建话题，指定话题失败不得回退默认话题。

`GET /api/admin/openclaw-notification-services/channels` 仅 Owner/Admin 可读，调用 Gateway `channels.status`，只返回 `channel/account_id/available`。管理员在 `/api/admin/openclaw-notification-services` 创建服务（name、openclaw_channel、openclaw_account、write-only destination、Telegram 可选话题），`/{id}` PATCH/DELETE，`/{id}/test-and-enable` 测试并启用。服务也出现在 `/api/notification-services`，其 channel 为 `openclaw`，只对自动化任务有效，不进入普通来源通知或告警绑定。Gateway 连接沿用服务端 SecretStore 与管理员设备；服务端直发 `send` RPC，不经过聊天模型。只有 Gateway 返回匹配渠道且带 messageId 的回执才标记 sent；超时或缺回执为 unknown，失败、未知均不能视作已送达。收件目标和 Gateway 凭据不得出现在列表、日志或浏览器响应。

既有库需离线显式执行 `scripts/migrate_notification_destinations_v47.py --data-dir ABS --apply --services-stopped`；预览不写库。global 47 新增 Telegram 话题表、OpenClaw 服务表和测试通知账本，保留旧目标、确认、回执与旧任务配置。新库 bootstrap 安装；旧库不自动迁移。
