...
21. 首次配置、第三槽补位与 legacy 升级都使用管理员手选的安全 Candidate allow-list。`GET /api/admin/apify-routes/{route_id}/pool-candidates?goal=...` 只返回同 workspace、同 Route、当前 Discovery Run 中可选 Candidate 的 opaque ID、公共名称、发布者、计费摘要、单次最高费用和人类不可用原因；`POST .../pool-candidates/refresh` 只创建免费 Discovery，不启动 Actor。管理员向 `POST /api/admin/apify-discovery-runs/{run_id}/canary-plan` 提交 `goal`、`candidate_ids` 集合、`expected_generation` 与固定 `target_slot_count=3` 生成无副作用计划；浏览器不得提交 Actor ID、Revision、Build、Manifest、槽位名、来源集合或输入。`initial_pool` 与 `upgrade_legacy` 必须恰好选择三个不同 Actor且覆盖至少两个发布者；`complete_third` 必须恰好选择一个与当前池不同的 Actor，现有 exact-Build Primary/Backup 1 为 probationary 或 certified 均可进入补位，Backup 2 必须为空。服务端在计划和付费提交时重新解析 Candidate 归属、状态与唯一 exact-Build Revision，任一 Build/Manifest 漂移、越权、重复 Actor、发布者不足或 Candidate 状态变化都 fail closed；绝不从管理员未选择的 Candidate 静默补位。
22. 三种 goal 都通过持久 Pool Stage 执行。plan hash 必须覆盖 goal、人工候选集合与服务端冻结顺序、服务端解析的 Actor/Build/Manifest、Route generation、基础 pool hash、当时所有 enabled source 的 binding generation/target fingerprint、逐项费用与批次上限。一次 `confirmation="确认付费验证主备"` 审批原子创建 batch、stage、候选 validation、来源快照和唯一 one-shot Job；Worker 严格按服务端冻结顺序先完成 Route Canary，再在同一批准上限内预验证快照中每个 enabled source 的目标槽。`initial_pool` 成功目标是新的完整 3/3；`upgrade_legacy` 在原 legacy 池持续服务时旁路验证三个 exact-Build Actor并最终整体替换为 3/3，旧 lifecycle 不得改写；`complete_third` 只验证并填入 Backup 2，原两槽全程不变。已通过付费验证的 probationary Revision 立即具备运行资格，后台认证观察不阻塞 staged apply。新增、binding 变化或失败来源使 stage 保持未就绪，下一 plan 只覆盖当前缺失证据并复用已有成功；禁用或删除来源不再阻塞。unknown-start 与费用待对账阻断后续付费和 apply，任何重试或增量计划都需要新的有界人工审批。
23. Staged apply 复用 `POST /api/admin/apify-routes/{route_id}/active-pool/activate`，并携带 `stage_id`、`expected_plan_hash`、`apply_id`、最新 Route generation 与既有确认词。服务在一个 `BEGIN IMMEDIATE` 中重新校验基础池、exact Build/Manifest、Actor/发布者多样性、batch 所有 item 与费用均已终结、无 inflight/unknown-start、以及当前所有 enabled source 对目标池都有匹配当前 fingerprint 的成功证据；任一条件变化必须零写入 fail closed。成功时原子写入完整目标槽、Route generation、来源 `ready_3of3` 与 stage 终态；精确 `apply_id` 重放返回同一结果，跨 stage/plan 复用冲突。Route detail 只投影服务端计算的 `workflow/next_action/progress/blockers` 和每个 Revision 的后台认证进度；当 staged approval 需要重新计划时，`workflow.progress.last_failure` 最多包含阶段、规范化安全错误代码、最终或待对账费用及终态标记，不得投影 candidate ID、来源 target/fingerprint、approval hash、远端 Run/Dataset ID、上游错误或内部输入。基础 Stage 能力由离线 migration `apify_actor_pool_staging_v18`（global version 20）安装；人工三选与三种 goal 由后续 `apify_actor_manual_pool_selection_v19`（global version 21）增加 `target_slot_count/selection_mode` 并把既有 Stage 安全回填为 `server` 模式。global 21 要求 global 20 已通过、API/Worker 停止且无活跃 ActorOps Job，使用 SQLite backup API 创建 `0600` backup并检查完整 marker/table shape、integrity 与 foreign keys；任何失败恢复备份，普通 initialize、API 与 Worker 不得静默升级，readiness 在显式迁移前返回 503。
24. Validation tuning 由离线 migration `apify_actor_validation_tuning_v20`（global version 22）安装。每个 Stage Candidate 冻结 `timeout_seconds=180..900`、`sample_items in (1,3,5)`、`max_charge_usd<=0.10` 与 profile hash；非 1 的样本数只在 Manifest 确实引用 `runtime.max_items` 时可用。plan hash、CAS、approval 幂等和 Worker Run 都使用同一冻结配置，不得回退到容器全局超时。Validation 持久化耗时、Dataset/有效行数和由 Route、opaque Candidate、exact Revision/Build/Manifest、加盐目标指纹、validation kind 与 profile 组成的失败指纹；同指纹新计划以 `409 apify_actor_validation_profile_unchanged` 在 Actor POST 前拒绝并记为 `$0`。超时只有增加等待才是有效调整，`suspicious_empty` 只有增加已支持的样本数才是有效调整，状态读取失败不得生成新付费计划。`POST /api/admin/apify-routes/{route_id}/validations/reconcile` 只按已知 durable reservation 重读原 Run/Dataset，不调用 Actor POST，也不 abort 仍在运行的 Run；批次恢复状态与续跑 Job 在同一事务内继续原审批已覆盖的剩余来源检查，不得扩大候选、费用或自动重试。global 22 需要 global 21，停 API/Worker、跨过 heartbeat 安全窗且无活跃 ActorOps Job，建立 `0600` backup、回填旧 Validation 为 300 秒/1 条/原审批上限，并通过 integrity/foreign-key/table-shape 后才提交 marker；失败必须恢复备份。
25. 第 24 条取代第 6 条中“超时只由容器环境变量热加载”与第 8 条中“每次固定 `maxItems=1`”的限制；容器变量只保留为旧流程默认值，Stage Validation 以数据库冻结 profile 为唯一权威。人工 Stage 的 Route 部分最多三个 Candidate 且每个最高 `$0.10`，因此 batch/route-validation 表约束上限为 `$0.30`；包含 enabled source 预验证的整个 Stage 仍受 `$6.06` 硬上限保护。
