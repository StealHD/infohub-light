...
- Selected context rows show readable source/person, one-line content, and time in draft order. New selections use the card display model immediately; migrated ID-only drafts resolve through the user-scoped `feedItem` query. Raw article IDs remain internal to session storage and deterministic MCP prompts, never visible UI copy. Loading or unavailable items degrade independently and remain removable.
