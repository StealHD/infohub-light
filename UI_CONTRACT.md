...
- At 1440×900, the workbench shows complete navigation, Feed, and a 360 px right rail with four to five complete collapsed cards. Feed initially uses the `信息概览` rail; Agent is an explicit peer mode. At 1024×768, either rail overlays without resizing or scrolling the Feed. At 390×844, bottom navigation and the selected right-rail sheet remain reachable without horizontal page overflow.
- Navigation, Feed, and the right rail have independent scroll regions. Right-rail state is exactly `closed | insights | agent`. Opening, closing, or switching modes preserves route, selected/expanded story, and Feed scroll anchor. Escape closes overlays and restores focus to their trigger.
...
- `/feed` exposes peer `信息概览` and `Agent` right-rail toggles with neutral hover/press feedback and a restrained accent selected state. On first wide-screen entry only, Insights may appear automatically; a manual choice remains authoritative. Adding context or sending opens Agent, and an active generation pins Agent so the stop control cannot be hidden. Insights reuses the latest Feed and source-health queries to show today's content, unread, saved, unhealthy-source, visible/total, channel, and content-format counts in compact Quiet Studio cards and bars; it never introduces a chart library or a new API.
...
- Browser chat states are `未连接/连接中/已连接/重连中/工具未发现`. A tab owns at most one Gateway WebSocket and one dedicated `Inteliscope` session, supports history, streaming, stop, new conversation and reconnect, and bounds visible history to the newest 100 messages and 100,000 text characters. Every send creates a concise user bubble, clears the pending question/context immediately, and sends a separate versioned MCP handoff prompt with a unique idempotency key and `deliver:false`; history projection recognizes both the versioned envelope and legacy handoff prompts without exposing internal instructions or article IDs.
