...
- Expanded navigation is organized as 浏览（信息流、收藏、历史）、常用视图（全部、未读、AI、朋友动态、产品机会）and 管理（订阅、助手连接、设置）. 稍后读 is absent. `全部` is the aligned baseline state: it clears unread/source/channel/topic/minimum-score filters while preserving the user's selected order. Other quick views only transform the existing user-isolated Feed preference and introduce no API or URL state.
...
- Feed has one mode, 全部. It defaults to `最新优先`, with newest content at the top; the explicit `最新优先`/`最旧优先` control persists per user and applies consistently to `/feed`, `/saved`, and `/history`.
...
- The centered content view bar aligns to the card column. Feed shows item count, order, manual refresh, filter, and active-filter count without recreating the removed search control. Saved and history place count, search, order, and filter in the same bar without a refresh action; mobile search expands from an explicit control. Filters include unread-first, local-day scope, source, channel, topic, and minimum score. Preferences remain user-isolated. `当天` follows the browser's local natural day, falls back from published time to fetched time, and recomputes at local midnight, focus, and visibility restoration. Filtering, quick views, ordering, and unread-first reordering preserve rendered-ID anchors.
...
- `/feed` exposes peer `信息概览` and `Agent` panel toggles with neutral hover/press feedback and a restrained accent selected state. Insights is one approximately 352 px Quiet Studio grouped card, never a width-owning third column. It may appear automatically once per browser session only when the measured empty gutter to the right of the 820 px reading frame is at least 376 px, Feed data is nonempty, and Agent is closed; it never covers a content card or takes focus. A manual close suppresses further automatic display in that session, while the toggle can always reopen it as an anchored surface or mobile Bottom Sheet. Opening Agent hides Insights without reopening it later; adding context or sending opens Agent, and an active generation pins Agent so the stop control cannot be hidden. The card follows its actual content height, omits empty distribution groups, shows the first three channel/type rows, and grows when the user explicitly chooses `查看更多`; only content exceeding the safe viewport height becomes vertically scrollable. Insights reuses the latest Feed and source-health queries to show today's content, unread, saved, unhealthy-source, visible/total, channel, and content-format counts in compact grouped rows; it never introduces a chart library or a new API.
...
- The connected composer summarizes context as a count plus at most two single-line source/title rows with fixed remove controls; `查看全部` opens a bounded popover containing all eight ordered rows. Long text ellipsizes, raw article IDs never become UI copy, and no context surface scrolls horizontally. New selections use the card display model immediately; migrated ID-only drafts resolve through the user-scoped `feedItem` query. Loading or unavailable items degrade independently and remain removable.
...
- Gateway bootstrap token or dashboard address exists only in the connection form state and is cleared after a successful connection. The browser stores only a non-exportable Ed25519 private key, exact `operator.read + operator.write` device token, and session key in IndexedDB, isolated by Inteliscope user and normalized Gateway URL. Broader roles/scopes are rejected. Logout disconnects and clears the session transcript; “忘记此浏览器” also deletes the local pairing and shows server-side device revocation guidance.
