...
- At 1440×900, the workbench shows complete navigation and a centered reading column with four to five complete collapsed cards; Agent opens as a 360 px fixed rail. On desktop the Agent rail is user-resizable from 320–720 px while preserving a minimum 640 px Feed region, and the clamped width persists only under `inteliscope.ui.right-rail.v1:<user_id>`. A 10 px accessible separator supports pointer drag, arrow keys, Shift acceleration, Home/End and double-click reset. At 1024×768 Agent overlays without resizing or scrolling the Feed. At 390×844, bottom navigation and the selected right-rail sheet remain reachable without horizontal page overflow.
...
- Browser chat states are `未连接/连接中/已连接/重连中/工具未发现`. A tab owns at most one Gateway WebSocket and one dedicated `Inteliscope` session, supports history, streaming, stop, new conversation and reconnect, and bounds visible history to the newest 100 messages and 100,000 text characters. Every send creates and immediately session-persists a concise user bubble, clears the pending question/context, and sends a separate versioned MCP handoff prompt with a unique idempotency key and `deliver:false`; history projection recognizes both the versioned envelope and legacy handoff prompts without exposing internal instructions or article IDs. Gateway history is incrementally reconciled by ID and normalized visible message identity and must never replace a local user message merely because Gateway history omitted it. The same user/Gateway/session fingerprint restores visible user and assistant messages after replies, refresh and ordinary reconnect; model forks migrate the visible transcript. Completed requests never persist the full MCP prompt, while pending/failed requests retain only the bounded retry snapshot. New blank conversation, logout and “忘记此浏览器” clear the applicable session transcript; ordinary disconnect does not.
