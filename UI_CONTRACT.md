...
- The centered content view bar aligns to the card column. Feed shows item count, order, manual refresh, filter, and active-filter count without recreating the removed search control. Saved and history place count, search, order, and filter in the same bar without a refresh action; mobile search expands from an explicit control. Filters include unread-first, source, channel, topic, and minimum score. Preferences remain user-isolated. Filtering, quick views, ordering, and unread-first reordering preserve rendered-ID anchors.
...
- Connected model choices come only from `models.list(view=configured)` and the active Agent/session returned by `agents.list` and `sessions.describe`. Searchable model and thinking controls patch only the dedicated Inteliscope session, are disabled during a run, roll back on Gateway rejection, and never change the OpenClaw global default. Missing thinking metadata degrades to the OpenClaw default instead of hard-coded levels.
