...
- Selected context rows show readable source/person, time, and one truncated content line in draft order. All eight bounded rows and their fixed trailing remove controls remain reachable without an independent context-list scroll; long source/content text shrinks and ellipsizes instead of increasing row height. New selections use the card display model immediately; migrated ID-only drafts resolve through the user-scoped `feedItem` query. Raw article IDs remain internal to session storage and deterministic MCP prompts, never visible UI copy. Loading or unavailable items degrade independently and remain removable.
...
- The disabled-mode `HandoffComposer` exposes a compact `交接模式` explanation, context count, model preference, transient live status, and labelled circular copy action. Its `CompactSelect` owns trigger value, indicator, popover, list items, keyboard behavior, Escape handling, and semantic `type-control` typography. Clipboard failure preserves the draft.
