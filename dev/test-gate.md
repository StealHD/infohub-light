...
当前为 10 个不同 CI 提交的观察期，默认完成门禁仍是 `full`。每次观察都必须满足 selector 无错误、`mapping_miss=false`、摘要与完整日志一致。达到 10 次后才能单独更新项目规则，将日常默认改为 `targeted`；PR/main 对受影响域运行完整门禁，文档-only main 只运行 control，正式发布复用精确 main SHA 的成功 Gate 并由 Tag 追加隔离 API smoke。
