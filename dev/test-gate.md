...
发布只执行：核对干净的本地 main、origin/main 和版本 → 复用同 SHA 镜像或本地构建一次 linux/amd64 镜像 → 核对 API/Worker 入口与产物身份 → 上传 → 将切换交给 VPS 独立 systemd 作业 → 停 API/Worker、保留其持久化的排队/运行任务并备份现有生产库 → 切换 → 版本/健康/React 静态资源检查 → 确认作业成功后创建并推送 Tag。SSH 断开时重新查询同一作业，不重复切换；结果未知时保留上传包和作业供核查，不发布 Tag。新 Worker 按原有租约/重试处理任务；只有显式数据库迁移才要求空队列。失败切换仍自动恢复旧程序。VPS 不编译项目；原有生产数据、运行配置和显式迁移边界沿用运行合同。
