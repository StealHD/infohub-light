...
- 2026-09-17 视觉补充：按用户提供的 HeroUI Pro 截图与 10 秒视频，完整 Agent 的进阶建议改用高比例卡片、窄边叠放、层级光影与空闲自动轮换；鼠标停留、键盘聚焦和 Reduced Motion 抑制自动轮换，手动切换重置计时。参数归组件合同。使用自有设计系统实现，不引入 Pro；选择后编辑再发送的语义不变。
...
2026-09-17 补充：用户指出诊断回复直接露出 Markdown 标记并要求修复。取代此前助手回复除链接外保持纯文本的 UI 规则：仅助手回复在完整工作区和紧凑 Agent 中安全排版常见 Markdown；用户消息仍按原文显示。当前流式片段与恢复历史共用渲染器，长代码和表格不横向溢出。只开放绝对 HTTP(S) 链接，原始 HTML 不执行，Markdown 图片仅显示替代文字且不加载外部媒体。解析器按需加载，避免扩大全站首屏包。行为真源为 [OpenClaw UI 合同](../../contracts/ui/workbench-feed-openclaw.md)。本次仅在当前分支实现并本地验证，未合并、推送或部署。
...
- 兼容收敛（2026-09-01）：VPS 旧数据可能没有可安全复用的 `source_native_title` 用户供体，但共享获取池仍保留规范中性条目；新订阅优先回填同 workspace 中性缓存，再回退到稳定索引。每次成功的 public/workspace 单源获取也在同一事务把安全中性内容投影给全部有效订阅者，因而不依赖每位成员各自创建 source schedule。回退和投影最多 200 条、零网络、零 AI，不读取 private isolation 或继承供体用户状态、AI 或行为状态。
