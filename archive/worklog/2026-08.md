...
```json
{
  ...
  "recorded_on": "2026-08-27",
  "result": "Unified Actor candidate failure classification, visible two-level status, target-aware manual replacement, and corrected Docker npm cache use.",
  "status": "partial",
  "task_id": "2026-08-27-actorops-failure-replacement",
  "unresolved": [
    "The final full gate was not rerun after its second maintenance-test failure per gate retry policy; the directly affected maintenance suite passes."
  ],
  "validation": [
    "ActorOps backend targeted suites passed",
    "ActorOps frontend Vitest, typecheck, ESLint, and three-viewport Playwright passed",
    "Impacted preflight passed before Dockerfile follow-up",
    "Local API and Worker rebuilt and healthy"
  ]
}
```
