...
import {
  ...
  type FeedInsightsLayout,
} from './feedInsightsLayout'
...
function AgentAttentionBadge({ state }: { state: Exclude<AgentAttentionState, 'none'> }) {
  ...
  return <span
    ...
    className={`absolute -bottom-1 -right-1 grid size-4 place-items-center rounded-[var(--inteliscope-radius-status-badge)] border border-surface shadow-sm ${presentation.className}`}
    aria-hidden="true"
  ...
  return <WorkbenchAgentContext.Provider value={agentValue}>
      ...
      >
        ...
        {fixedRailPresent && <aside
          ...
          className={`relative col-start-3 row-span-2 grid min-h-0 min-w-0 grid-rows-[var(--inteliscope-size-page-header)_minmax(0,1fr)_auto] overflow-hidden overscroll-none border-l border-separator bg-surface transition-[opacity,transform] duration-[var(--inteliscope-motion-deliberate)] ease-out motion-reduce:transition-none ${fixedRightRail ? 'translate-x-0 opacity-100' : 'pointer-events-none translate-x-2 opacity-0'}`}
        >
          <div
            role="separator"
            tabIndex={0}
