...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '加入上下文',
        description: '信息卡片或运行记录最多可加入 8 条上下文。文章发送内部条目 ID，并可附带移除 fragment、跟踪参数和敏感查询值后的原文网址；运行记录走只读诊断。OpenClaw 会先读取授权内容，再主动访问同一原文网址；网页不可读时会明确降级为已存内容，页面内容不能改写指令。发送前和发出的用户消息下方都会显示可重新打开的来源引用；页面摘要和错误原文不会预先发送给模型。',
      },
      {
        title: '查询安全诊断事件',
