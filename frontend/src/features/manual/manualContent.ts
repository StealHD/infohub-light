...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        href: '/settings/legacy#settings-fetching',
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '工作区设置',
        description: '进入设置后，左侧会完整替换应用主侧栏，并在顶部提供“返回应用”；返回时会恢复打开设置前的页面、查询条件和定位，直接打开设置链接时则回到信息流。桌面端按概览、工作区 / 来源与已忽略内容、智能 / AI、通信 / 通知、系统 / 外观和开发者 / 密钥与高级分组；手机端从顶部菜单打开同一套全高侧栏。概览、外观、通知、AI、已忽略内容和密钥均使用独立页面：概览不会额外读取业务接口，外观与右上角明暗模式按钮同步，通知继续使用原有服务配置和个人通知逻辑。Owner/Admin 可在 AI 页面配置模型、Key 元数据与触底文案，高级 Base URL、语言和长度限制默认折叠；Member/Viewer 只看到说明，不会读取工作区 AI 配置或 Key。已忽略内容最多读取 200 条并可逐项恢复到信息流。密钥页只显示名称、Provider、环境变量和使用关系；新增、轮换或失败时真实 Key 都会立即清空且永不回显。Apify Key 池会显示主备顺序、安全运行状态和安全额度投影，可手动刷新；排空期间会自动更新，运行中或排空中的 Key 必须先安全排空才能轮换或删除。来源会转到订阅工作区；只有获取与主题、存储与归档保留在兼容页面，原有权限、草稿、局部/全部保存和 write-only 安全规则均不改变。抓取/筛选仍可设置近 7、14 或 30 个上海自然日的工作区 Feed 范围。Bilibili 等受控来源只保存站点、路由和 UID，运行时使用这里配置的 RSSHub；自建公网实例可把访问密钥写入 SecretStore。收件地址、Webhook、Telegram Chat ID、Bot Token 和真实 Key 保存后都不会回显。',
        href: '/settings',
        linkLabel: '打开设置',
      },
      {
