...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '只通知之后的新内容',
        description: '先在设置页“通知目标”中新增一个邮箱、Webhook 或 Telegram 会话，保存后发送测试并启用；以后个人新内容通知和其他业务只选择目标，不需要重复填写目的地或重复测试。你可以选择自己的私有目标，也可以选择管理员提供的共享目标，还可以同时选择多个相同渠道目标。Webhook 支持通用事件 JSON、通用文本 JSON、飞书/Lark V2、企业微信群机器人、钉钉、Slack/GovSlack 或 Discord；Telegram 目标使用管理员已经测试并启用的工作区 Bot 服务。随后直接使用订阅卡片上的 Bell/BellRing 图标开启来源通知。目标、基础服务、总开关或订阅停用期间发现的内容都不会补推；personal_only 来源不参与通知。',
        href: '/settings',
        linkLabel: '配置消息通知',
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        description: 'Owner/Admin 可在独立的“发送基础服务”中并列配置“邮件发送服务”和“Telegram Bot 服务”。邮件选择 QQ、网易、Gmail、Resend 或 Amazon SES，并依次“保存配置 → 发送测试邮件 → 启用”；Telegram 保存 Bot Token 后同样先向一次性测试 Chat ID 发送测试，再启用当前 generation。凭据、Bot Token 与测试目的地提交后立即清空且不会回显，修改后必须重新测试。暂停或删除一种 Transport 只会让依赖它的通知目标暂不可用，其他目标继续发送；成员只在统一“通知目标”中维护自己的收件邮箱或 Telegram Chat ID。管理员应只把共享 Bot 加入获准会话；目标 Chat ID 的可投递范围由 Bot 在 Telegram 中已经拥有的成员与发言权限决定，并不是站内新增的会话审批名单。',
        href: '/settings',
        linkLabel: '配置通知发送服务',
      },
    ],
