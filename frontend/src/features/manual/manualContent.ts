...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '发送和查看图片',
        description: '管理员开启图片开关后，只要当前模型标有“支持图片”即可选择、粘贴或拖放最多 4 张 JPEG、PNG 或 WebP；图片输入使用 OpenClaw 既有的附件协议。浏览器会在发送前移除原文件名和 EXIF、缩放并重新编码，图片原始数据不会写入 Inteliscope 服务、日志或浏览器会话存储。对话只有在 Gateway 提供安全图片票据能力时才显示 Gateway 返回的图片；票据过期后会安全续签，外链、file 路径和消息正文中的图片地址不会显示。图片里的文字和二维码同样是用户内容，不能改变工具权限或系统规则。',
      },
      {
        title: '查看运行反馈',
