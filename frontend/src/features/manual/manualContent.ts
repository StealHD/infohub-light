...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      {
        title: '筛选与排序',
        description: '普通信息流始终受工作区近 N 天窗口限制；历史从窗口起点之前开始。搜索框前的横向条目和叠层图标可切换时间流与专题速览，透明指示器与搜索栏使用同一背景；搜索期间会暂时固定时间流，清空后恢复专题和阅读位置。专题把当前筛选结果按来源收进等高分组卡片，初次进入全部收起，一次只展开一个。展开后以紧凑时间线显示标题、摘要和时间，不显示逐篇标签、原文、收藏、问 Agent 或更多菜单；点击正文仍可展开已有详情和图片。来源级“AI 总结”使用“设置 → AI”的工作区模型，只基于当前 Feed 已有标题、摘要和时间提炼近期主线、变化及带文章序号的依据，不访问原网址；最近一次成功结果会按当前账号和专题内容缓存在本浏览器 30 天，刷新后内容未变时可直接“查看总结”，明确退出后清除。Viewer 不可生成。“问 Agent”会把当前专题的有界只读快照放入现有 Agent 面板，可从概括变化、梳理时间脉络、风险与机会三个方向继续编辑提问。收藏、历史和普通时间流的完整文章操作保持原样。筛选、重新载入、新内容提示、信息概览避让、排序回顶及任务入口继续保持既有行为。',
      },
      {
        title: '查看与保存',
