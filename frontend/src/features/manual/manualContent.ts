...
export const manualReview = {
  ...
  change: '自动化模型选择改为直接读取当前用户的 OpenClaw 已配置模型；目录读取不再要求分析执行器，也不调用模型。',
} as const
...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '只通知之后的新内容',
        description: '先由 Owner/Admin 在设置页“通知服务”中配置邮箱、Webhook、Telegram 或自动化专用 OpenClaw 服务，并点击一次“保存并测试”；确认成功后会自动启用。以后个人新内容通知和其他业务只选择服务，不需要重复填写目的地或重复测试，还可同时选择多个相同渠道服务。历史私人目标仍只对原用户可见并可继续选择。Webhook 支持通用事件 JSON、通用文本 JSON、飞书/Lark V2、企业微信群机器人、钉钉、Slack/GovSlack 或 Discord。随后直接使用订阅卡片上的 Bell/BellRing 图标开启来源通知。服务、总开关或订阅停用期间发现的内容都不会补推；personal_only 来源不参与通知。',
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '手动测试与模型刷新',
        description: '模型目录直接读取当前用户个人 Agent 已配置且可用的 OpenClaw 模型；点击“刷新模型目录”只做一次读取，不调用模型、不启动执行器，也不修改当前选择。目录可用与实际测试可执行分别校验：只有提交测试或启用任务时才检查独立执行能力。读取失败时检查本机 Gateway 连接和管理员授权后重试。OpenClaw 已返回模型调用失败时，测试会停止并可手动重新测试；只有连接中断或超时等无法确认终态的情况才需要先核对分析记录。',
      ...
      {
        title: '个人信息提醒',
        description: 'Automations 可按状态筛选，或搜索已加载任务的名称、描述与来源。点击任务先看概览，再按需进入编辑、测试或运行记录；宽屏详情可拖动并按账号记忆宽度，空间不足使用抽屉或底部面板。编辑时来源在独立面板中搜索和确认，模型可直接选择，触发与通知按需展开；退出编辑、切换标签或刷新列表仍保留草稿，保存不等于启用。已归档任务可恢复为草稿，恢复不会自动启用或发送。测试可从当前最新 Feed 中选择符合任务来源的文章，也可直接输入自定义文本；有自定义文本时只使用该文本，不读取或改写 Feed、正式水位和任务配置。默认不通知，显式勾选后仅分析命中才发送“测试通知”，本次可临时改选已验证服务且不改变正式配置；关闭详情或查看其他任务后，列表仍显示当前页面内的测试状态，重新打开可继续查看。结果先显示综合结论，原因、原文依据和逐篇结果按需展开；提交结果未知时不会自动重试。新建任务时，用完整描述填写关注内容、关键词与排除要求，绑定订阅源并选择 OpenClaw 模型；通知默认关闭，可选择已验证服务后开启，关闭通知仍会保存分析结果。可按每条到达、累计条数、固定间隔、每天或每周固定时间触发；累计模式可设置最长等待。大批量分段分析后发送一条批次通知；模型失效保留队列，刷新模型或仅更换模型并重新确认后恢复。修改其他条件需重新确认，不补发暂停期间历史。原订阅全量通知独立生效。未知发送结果不自动重发，高级 Gateway Cron 保留独立入口。',
      ...
      },
      {
        ...
        description: '从左上角产品切换器选择“OpenClaw — 对话、执行与自动化”，或在信息流 Agent 右栏点击“在 Agent 工作台打开”。页面会保留当前 Gateway 连接、会话、生成状态、对话和 Feed 草稿；进入助手连接、账户或设置也不会因路由拆除 Runtime。左侧最多显示三条单行会话；“全部会话”可搜索和分页查看当前 Gateway 授权的历史会话，也可切换到已归档记录。Tasks/Artifacts 仍只读取可信根会话及完整 parentSessionKey 来源链，不因历史目录扩大范围。新对话首条消息后可自动生成总结标题，旧名称不会批量改写。生成结束或取消后才可切换。Tasks 与 Artifacts 作为按需检查器打开且不卸载对话，文件不会经过 Inscope API。',
        href: '/agent',
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '管理员统一配置通知服务',
        description: 'Owner/Admin 在“消息通知”的单一“通知服务”表格中配置和维护工作区共享服务。桌面显示服务、渠道、状态、使用情况和操作；窄屏将次要信息收进服务列，仍不横向滚动。新增和编辑使用弹窗，行操作收进“更多”菜单，归档会要求再次确认。首次创建 Telegram 服务时同时填写服务名称、Chat ID 和共享 Bot Token，可选填正整数话题 ID，留空发送到默认话题；之后可复用已配置 Token，并可编辑时保留、更换或清除话题；首次创建邮件服务时同时选择 QQ、网易、Gmail、Resend 或 Amazon SES、填写发送凭据和收件地址，之后可复用发送凭据；Webhook 直接填写 Provider、URL 和可选签名。OpenClaw 服务选择 Gateway 已配置的渠道与账号，填写收件目标；Telegram 渠道也可选填话题，仅自动化可选择此类服务。唯一主操作是“保存并测试”，接收端确认成功后自动启用，失败则保留不回显秘密的停用草稿供修改或重试。凭据、Bot Token、Chat ID、收件地址、Webhook URL 与签名提交后立即清空且不会回显。管理员应只把共享 Bot 加入获准会话；Chat ID 的可投递范围由 Bot 在 Telegram 中已经拥有的成员与发言权限决定。',
        href: '/settings/notifications',
        linkLabel: '配置通知发送服务',
      },
    ],
