...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '验证 Actor 来源',
        description: 'X、Instagram 和 YouTube 付费回退由“设置 → ActorOps”统一管理。先选择抓取类型，再按“主备配置 / 来源启用 / 运行与告警”完成任务；页面只显示当前主备和一个明确的下一步，不要求理解 Discovery、Job、Revision 或机器状态。首次配置和旧版升级从安全候选列表选择 3 个 Actor，补备用 2 选择 1 个；候选只显示名称、发布者、适用性与最高费用，没有粘贴 Actor ID 的入口，精确 Build 和槽位顺序由服务端固定。更新候选免费且不会启动 Actor；正常流程只需“确认验证（最高 $X）”和“确认生效”两次确认。验证期间旧版池或现有两路继续运行，全部已启用来源预验证完成后，第二次确认才原子切换完整 3/3 或只补入备用 2。Actor 通过付费验证后已经“已验证，可运行”，可以立即生效；参考来源、48 小时观察和至少 95% 成功率由系统在后台继续认证，不会卡住配置，也没有手工转正按钮。若候选失效、内容不兼容、超时、费用待对账或启动结果未知，页面会直接说明原因、影响与下一步，绝不自动重发付费请求。YouTube 仍先使用免费原生 Feed，只有允许回退的网络或合同故障才进入 Actor Route。',
