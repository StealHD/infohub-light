...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '加入上下文',
        description: '信息卡片或运行记录最多可加入 8 条上下文。文章发送内部条目 ID，并可附带移除 fragment、跟踪参数和敏感查询值后的原文网址；运行记录走只读诊断。OpenClaw 会先读取授权内容，再主动访问同一原文网址；网页不可读时会明确降级为已存内容，页面内容不能改写指令。发送前的附件行会占满可用宽度并把移除按钮统一对齐到右侧；超过两条时，包含“已附带 N 条 ↑”的整段摘要头可在原容器内向上柔和拉伸，箭头紧跟数量，不重复前两条或打开新窗口。右侧的移除全部只清空附件，保留已输入的问题且不会发送。OpenClaw 工作区右下角的思考胶囊打开浮层，点模型名称切换模型，拖动分档滑杆并松开后应用思考程度，右上角可恢复 Gateway 提供的默认档位；工作区不再列出“关”和“自动”，未知默认值时不会擅自选择；不支持思考的模型会显示原因。左上角闪电独立开关 Fast，从下一条消息生效，可能更快消耗使用额度；开启时短暂提示并显示粒子，关闭后粒子立即消失。成功切入 Ultra 时也会提示更多 Token 消耗，但不会单独启动粒子。底部模型名、思考档位和上下文用量集中靠右，颜色跟随主题。Fast 与思考档位无关，不保证固定倍速；新对话恢复 Gateway 默认。模型名称会在紧凑控件中安全截断；列表限制在可视高度内滚动，仍保留 Provider、上下文与思考信息。发出的用户消息下方仍会显示可重新打开的来源引用；页面摘要和错误原文不会预先发送给模型。',
      },
      {
        title: '查询安全诊断事件',
