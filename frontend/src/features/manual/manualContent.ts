...
export const manualSections: ManualSection[] = [
  {
    ...
    steps: [
      ...
      {
        title: '开始阅读',
        description: '回到信息流后，可用“刷新”只读取服务器上的最新快照，也可用“更新”启动后台来源抓取。“刷新”进行中按钮保持原位，只旋转图标并阻止重复点击。还可按发布时间或入库时间排序，并使用当天、公共订阅、私人订阅等常用视图缩小范围。',
        href: '/feed',
        linkLabel: '打开信息流',
      },
    ],
  },
  {
    ...
    steps: [
      {
        title: '筛选与排序',
        description: '筛选、刷新数据、后台更新和常用视图会尽量保留当前卡片位置；离开新鲜边缘时，新到内容会通过“N 条新内容”提示等待查看。切换排序方向或时间口径会明确回到列表顶部。',
      },
      {
        title: '查看与保存',
        description: '卡片可以从标签或空白区域展开，也可用展开/折叠线框按钮控制；鼠标或键盘停留在底部操作上时，说明会优先显示在按钮上方。图片会完整显示在缩略格中，点击后可查看全图并用左右按钮或方向键切换。也可以打开原文、收藏或加入 Agent 上下文，忽略的内容只能从设置页恢复。',
      },
      {
        title: '管理收藏',
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '工作区设置',
        description: '设置页集中管理主题、消息通知、恢复已忽略内容以及管理员可见的 AI、Key 与 RSSHub Base URL。Bilibili 等受控来源只保存站点、路由和 UID，运行时使用这里配置的自建、反向代理前缀或第三方 RSSHub；自建公网实例可把 RSSHUB_ACCESS_KEY 写入 SecretStore，Worker 只发送路由级 code，OpenClaw 不会看到或提交服务地址与密钥。收件邮箱或 Webhook 保存后只显示是否已配置，测试通知使用模拟内容，不抓取来源也不改变新内容起点；真实 Key 同样不会回显。',
        href: '/settings',
        linkLabel: '打开设置',
      },
      {
