...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      {
        title: '配置个人 Agent',
        description: 'Owner/Admin 在“我的 Agent 接入”点击“配置个人 Agent”，确认后准备独立绑定并下载私密配置包。按窗口指引在受信任 OpenClaw 主机安装、重载并验证，再上传一小时内生成的 receipt.json 完成接入；途中关闭可点“继续配置”。数据连接不等于 Agent 绑定，不必删除已有连接。普通成员由管理员通过运维工具接入；失效或撤销的身份也需运维恢复。配置包含本人只读令牌，不要分享或提交 Git；模型沿用 OpenClaw 配置，此流程不调用模型或发送通知。',
