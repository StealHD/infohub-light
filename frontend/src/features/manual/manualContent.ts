...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '只通知之后的新内容',
        description: '先在设置页保存收件邮箱或 Webhook 并启用消息通知，再直接使用订阅卡片上的 Bell/BellRing 图标开关；鼠标停留或键盘聚焦会在上方说明当前状态，订阅设置弹窗不再重复该选项。未启用签名校验的飞书/Lark V2 自定义机器人地址会自动使用群机器人文本消息格式，机器人关键词和 IP 白名单仍需放行；当前没有签名密钥配置入口。其他 HTTPS Webhook 继续接收通用 JSON 事件。邮箱还要求管理员已启用工作区发件服务；服务暂停时保留你的选择，但不入队也不补发。首次快照、历史或复用内容、停用期间发现的内容都不会补推；personal_only 来源不参与通知。',
        href: '/settings',
        linkLabel: '配置消息通知',
      },
      {
