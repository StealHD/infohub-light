...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      {
        title: '筛选与排序',
        description: '普通信息流始终受工作区近 N 天窗口限制；历史严格从窗口起点之前开始，两者不会重叠或遗漏。信息流工具栏的搜索框前有两个小图标：横向条目图标表示时间流，叠层图标表示专题速览，停留时可查看名称。时间流保持按时间连续阅读；专题速览会把当前筛选结果按来源收进独立的分组卡片，并显示近 N 天文章数、主题数和高频主题。专题初次进入全部收起，点击来源标题展开；一次只展开一个来源，方便连续扫读而不被其他专题打断。选择只保存在当前账户的浏览器中；收藏和历史仍保持原来的时间流。信息流的全部内容搜索、收藏搜索和历史搜索相互隔离，不会在切换页面后暗中套用；搜索期间会暂时固定为时间流，清空后恢复原来的专题和阅读位置。筛选、重新载入、获取新内容和常用视图会尽量保留当前卡片或来源标题位置；离开新鲜边缘时，新到内容会通过“N 条新内容”提示等待查看。切换排序方向或时间口径会明确回到列表顶部。“信息概览”打开后会为阅读列留出空间；还可查看我的订阅、来源库与最近加载的最多 20 条信息流相关任务。',
      },
      {
        title: '查看与保存',
