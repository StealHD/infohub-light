...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '工作区设置',
        description: '进入设置后，左侧会完整替换应用主侧栏，并在顶部提供“返回应用”；返回时会恢复打开设置前的页面、查询条件和定位，直接打开设置链接时则回到信息流。桌面端按概览、工作区 / 来源、智能 / AI、通信 / 通知、系统 / 外观和开发者 / 高级分组；手机端从顶部菜单打开同一套全高侧栏。第一阶段的概览、外观和通知已经使用独立页面：概览不会额外读取业务接口，外观与右上角明暗模式按钮同步，通知继续使用原有服务配置和个人通知逻辑。来源会转到订阅工作区；AI、恢复已忽略内容、获取与主题、存储与归档及密钥暂时保留在兼容页面，原有权限、草稿、局部/全部保存、五分钟 Apify 额度缓存与 write-only 安全规则均不改变。首次打开“密钥”会自动读取已配置 Apify Key 的安全额度投影，仍可手动刷新。“助手与 AI”中的“信息流触底文案”仍可独立配置生成周期、风格和每场景数量；抓取/筛选仍可设置近 7、14 或 30 个上海自然日的工作区 Feed 范围。Bilibili 等受控来源只保存站点、路由和 UID，运行时使用这里配置的 RSSHub；自建公网实例可把访问密钥写入 SecretStore。收件地址、Webhook、Telegram Chat ID、Bot Token 和真实 Key 保存后都不会回显。',
        href: '/settings',
        linkLabel: '打开设置',
      },
      {
