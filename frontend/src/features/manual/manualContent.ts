...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '个人信息提醒',
        description: 'Automations 可按状态筛选，或搜索已加载任务的名称、描述与来源。列表和详情可直接启动完整草稿或已暂停任务，也可暂停已启用任务；启动只处理之后的新内容，不补跑历史或自动测试。点击任务先看概览，再按需进入编辑、测试或运行记录；概览底部会显示最近三次真实运行，完整结果在运行记录查看。宽屏详情可拖动并按账号记忆宽度，空间不足使用抽屉或底部面板。编辑时来源在独立面板中搜索和确认，模型可直接选择，触发与通知按需展开；退出编辑、切换标签或刷新列表仍保留草稿，保存后才能启动。已归档任务可恢复为草稿，恢复不会自动启用或发送。测试先从当前最新 Feed 中选择符合任务来源的文章，不会自动选择、自动执行或发送通知；关闭详情或查看其他任务后，列表仍显示当前页面内的测试状态，重新打开可继续查看。结果先显示综合结论，原因、原文依据和逐篇结果按需展开；提交结果未知时不会自动重试。新建任务时，用完整描述填写关注内容、关键词与排除要求，绑定订阅源并选择 OpenClaw 模型。可按每条到达、累计条数、固定间隔、每天或每周固定时间触发；累计模式可设置最长等待。大批量分段分析后发送一条批次通知；模型失效保留队列，刷新模型或仅更换模型后重新启动即可恢复。修改其他条件会暂停任务，不补发暂停期间历史。原订阅全量通知独立生效。未知发送结果不自动重发，高级 Gateway Cron 保留独立入口。',
