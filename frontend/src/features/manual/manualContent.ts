...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '手动测试与模型刷新',
        description: '新安装默认只执行手动测试，不领取正式自动化。目录可用与测试可执行分别校验；离线、执行关闭或模型不可用时按提示修复。恢复前的旧测试需要点击确认重新测试，已经领取且结果未知时先核对分析记录。刷新页面可恢复最近测试。新增模型后点击刷新模型目录，等待同步完成；无变化也会明确提示，120 秒未收到回执则显示超时，请检查服务后手动重试。刷新不修改当前选择，不调用模型验证。管理员明确设置的白名单不会被覆盖，旧白名单归属不明时需管理员核对。',
        href: '/agent/automations', linkLabel: '打开自动化',
      },
      {
        title: '修复聊天工具或自动化模型目录',
        description: '已接入但个人工具或自动化不可用时，Owner/Admin 在我的 Agent 点击“修复接入”；成员由管理员在已接入申请行修复，无需解除后重建。系统复用原绑定和正常凭据，核验个人工具协议并安装独立分析服务。聊天连接、个人数据授权和分析服务分别显示状态，连接成功不等于模型已验证。目录未配置时刷新不能代替修复；离线或过期由管理员检查托管服务。模型切换期间不能发送，旧请求不兼容时保留内容，不自动换模型重试。分析结束状态无法确认时停止领取，撤销也不误报清理完成。',
