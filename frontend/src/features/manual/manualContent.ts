...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '验证 Actor 来源',
        description: 'X、Instagram 以及 YouTube 的付费回退由“设置 → 获取与主题”中的 ActorOps 统一管理。首期支持检查只提供 X Profile、YouTube Channel 与 Instagram Profile 三种完整组合，不允许把 YouTube 与 Profile 等目标类型任意拼接。每条 Route 固定显示 Primary、Backup 1、Backup 2；Discovery 会在一次 AI 调用中排序并尝试 3–6 个候选，已经通过的部分结果会显示为 1/3 或 2/3 并继续等待补位，不会因暂时不足三槽被隐藏，但达到三个 Actor和两个发布者前不能付费 Canary。目标 URL、handle 或原生 ID 的输入形状由代码从官方 Build Schema 生成，AI 不再猜测 string、array 或 startUrls object；每个输出字段路径也必须能在精确 Build 的 Dataset Schema 中验证，账号或频道内容不能把帖子 URL 当作来源身份。时间映射支持带时区 ISO 以及有界 Unix 秒/毫秒；Actor 先返回账号元数据、后返回真实帖子时会隔离元数据行并继续验证，全为元数据才安全失败。官方输入或静态校验拒绝某个候选时只淘汰该候选并继续。页面会区分 Store/元数据故障、输入不兼容、临时校验故障和真正的候选不足。新账号必须按顺序让三槽各完成一次来源级 Canary，三次都通过后才能首次启用。每次付费试跑和首次启用都需要管理员单独确认；确认页会先列出 Route、来源类型、Actor、Build、商城价格、本次封顶和认证总预算。Canary 默认最长等待 300 秒，超时会中止已知 Run且不自动重试，实际费用从远端终态账本对账；一轮五次均未凑齐三个成功 Revision 时停止审批并要求强制重新发现。Actor、Build、Manifest、顺序、Route 费用和 Discovery 输出 Token 上限修改后无需重启；Actor Discovery 继承“助手与 AI”中已保存的全局 provider、model 与 Base URL，并由管理员在同 Provider 的 Key 中人工选择一个，不另存模型配置，也不会自动换用其他 Key。管理员输入“确认AI容量测试”后，可顺序测量 YouTube/Instagram 的模型用量；32K 只用于本次护栏，64K 仅在输出被截断后重测，测试不会启动 Actor 或付费 Canary。页面只显示安全 Token、耗时和验证状态，未知用量不会显示为 0，建议值也不会自动保存。全局 AI 不可用时只能阻断新发现，既有 Route 继续运行。X 已启动 Run 的 Dataset 解码失败只会重读同一 Dataset，不会重新付费启动或切换 Actor。新 Actor、Build 或 Manifest 仍必须通过静态校验、付费 Canary 和管理员激活。YouTube 始终先使用免费原生 Feed，只有允许回退的网络或合同故障才进入 Actor Route。',
      ...
      {
        title: '只通知之后的新内容',
        description: '先在设置页保存收件邮箱或 Webhook 并启用消息通知，再直接使用订阅卡片上的 Bell/BellRing 图标开关；鼠标停留或键盘聚焦会在上方说明当前状态，订阅设置弹窗不再重复该选项。Webhook 可选择通用事件 JSON、通用文本 JSON、飞书/Lark V2、企业微信群机器人、钉钉、Slack/GovSlack 或 Discord；通用两类只需要 HTTPS 地址，不支持 Bearer Token 或自定义请求头，飞书和钉钉可按机器人设置填写签名 Secret。平台预设会核对平台业务响应，通用类型只确认 HTTP 2xx；保存成功只表示配置写入，发送测试后仍请确认接收端实际展示。升级前保存的地址会继续兼容发送，但修改时必须重新选择类型并输入对应地址。邮箱还要求管理员已启用工作区发件服务；服务暂停时保留你的选择，但不入队也不补发。首次快照、历史或复用内容、停用期间发现的内容都不会补推；personal_only 来源不参与通知。',
        href: '/settings',
        linkLabel: '配置消息通知',
      },
      {
