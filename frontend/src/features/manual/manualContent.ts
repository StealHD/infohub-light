...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '直接操作与稳定获取状态',
        description: '允许编辑的来源会在卡片下方紧凑控制行显示 Pencil 图标，和通知、订阅设置、立即获取放在同一操作区；卡片标题区右上保留健康状态，状态下方紧凑显示“今日 / 近N天 / 历史”。历史大于零时可直接打开该来源的历史内容。私人来源所有者会在同一行直接看到“分享来源”，无需再打开只有一个动作的“更多”；关闭或完成分享后焦点回到该按钮。底部仅用内联文字显示“更新：全局 / 单源 / 跟随全局 / 订阅已停用”、实际周期和下次时间，不再把默认状态误写成“手动更新”。“上次抓取”只放在更新时间和立即获取的说明中，不再冒充可见数量。信息流出现“查看 N 条新内容”时，按钮固定在顶部工具栏下方，不会遮挡筛选或排序操作；移动端信息会自然换行且不横向滚动。“立即获取”在提交、排队和运行期间保持同一宽度与文字，只旋转图标并阻止重复点击。',
      },
      {
        title: '排查失败',
  ...
  },
  {
    ...
    steps: [
      {
        title: '创建数据连接',
        description: '在“助手连接”创建只读或订阅管理连接。令牌只显示一次；创建流程的令牌、环境写入命令和 OpenClaw 配置命令都用紧凑图标复制，长令牌或命令会自动换行，不会撑出横向滚动。复制环境写入命令后，在本机终端粘贴运行即可安全更新 ~/.openclaw/.env 中的 Inteliscope 令牌，保留其他环境变量并设置目录 0700、文件 0600；网页不会直接执行本机命令。订阅管理仍需 prepare、确认和 apply，不能直接改写数据。现有连接的复制、重命名、吊销或删除集中在中性“更多”菜单；吊销只出现在有效连接，删除只出现在已吊销记录，红色实心动作只会出现在最后一步确认。',
        href: '/agents',
        linkLabel: '打开助手连接',
      },
      {
