...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '跨浏览器使用',
        description: '历史或归档会话的行尾 X 会在鼠标悬停或键盘聚焦时显示，触屏始终可见；停留或聚焦可看到“删除会话”。切换到其他会话后，点击 X 并确认才会由 Gateway 永久清理；主会话和运行中会话不可删。同一账号默认允许 12 个页面同时连接；关闭标签页或离开站点会释放连接，浏览器后退恢复时重连，切到后台不主动断开。连接不会因满一小时被强制切断。部署管理员可通过 HORIZON_OPENCLAW_MAX_CONNECTIONS_PER_USER（1–100）调整上限并重启 API。即使共用账号，每人仍共享同一 Agent 身份。同一账号的个人 Agent 绑定由服务端保存；换浏览器只需登录、连接，不重复申请 Agent。浏览器仅保存既有会话选择和连接偏好，不保存 MCP 令牌、Gateway 管理凭据或回执。未接入时，聊天和设置统一前往 Agent 接入页。原会话与授权不会被自动删除，撤销对所有浏览器生效。A 模式使用本机前端 5173、API 8080 和原测试库，不需要 Docker。',
        href: '/agents',
        linkLabel: '打开助手连接',
      },
      {
      ...
      {
        title: '个人信息提醒',
        description: 'Automations 可按状态筛选，或搜索已加载任务的名称、描述与来源。列表和详情可直接启动完整草稿或已暂停任务，也可暂停已启用任务；启动只处理之后的新内容，不补跑历史或自动测试。点击任务先看概览，再按需进入编辑、测试或运行记录；概览底部会显示最近三次真实运行，完整结果在运行记录查看。宽屏详情可拖动并按账号记忆宽度，空间不足使用抽屉或底部面板。编辑时来源在独立面板中搜索和确认，模型可直接选择，触发与通知按需展开；退出编辑、切换标签或刷新列表仍保留草稿，保存后才能启动。已归档任务可恢复为草稿，恢复不会自动启用或发送。测试可从当前最新 Feed 中选择符合任务来源的文章，也可直接输入自定义文本；有自定义文本时只使用该文本，不读取或改写 Feed、正式水位和任务配置。默认不通知；显式勾选后仅分析命中才发送“测试通知”，本次可临时改选已验证服务且不改变正式配置。关闭详情或查看其他任务后，列表仍显示当前页面内的测试状态，重新打开可继续查看。结果先显示综合结论，原因、原文依据和逐篇结果按需展开；提交结果未知时不会自动重试。新建任务时，用完整描述填写关注内容、关键词与排除要求，绑定订阅源并选择 OpenClaw 模型；通知默认关闭，选择已验证服务后可开启，关闭时仍保存分析结果。可按每条到达、累计条数、固定间隔、每天或每周固定时间触发；累计模式可设置最长等待。大批量分段分析后发送一条批次通知；模型失效保留队列，刷新模型或仅更换模型后重新启动即可恢复。修改其他条件会暂停任务，不补发暂停期间历史。原订阅全量通知独立生效。未知发送结果不自动重发，高级 Gateway Cron 保留独立入口。',
