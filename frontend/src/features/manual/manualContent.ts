...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '成员申请与审批',
        description: '已启用成员在 Agent 接入页点击“申请接入”，无需填写理由或配置。Owner/Admin 在同页“接入申请”中筛选待审核、处理中或已处理，搜索申请人并允许或填写原因拒绝。允许后后台配置成员独立 Agent，验证完成才可进入聊天；失败由管理员重试，不要求成员重新申请。拒绝后可以重新申请。状态服务端保存，不发送通知，成员主动查看结果；Viewer 不开放申请。',
      ...
      {
        title: '连接本地 Gateway',
        description: '浏览器直接连接用户自己的 OpenClaw Gateway。切换本地端口或浏览器后，需要为当前地址完成首次配对：运行 openclaw gateway auth-token --show，在本机终端查看当前 token，再填写到连接表单。只有当前地址已经配对，才能使用“使用已配对设备重连”。多套本地环境各自固定前端端口和对应 API 代理目标，并将实际页面来源加入 Gateway 允许来源；不要依赖默认端口。服务端模式由管理员为每个账号部署独立 Agent 和 MCP 授权，Owner/Admin/Member 绑定就绪后可聊天，Viewer 只读。绑定未配置、过期或吊销时需要修复个人接入；旧 main 会话保留本人只读历史。用户 MCP 连接统一跟随账号角色：管理员可管理订阅、系统设置并查询工作区诊断，成员可管理本人订阅，Viewer 只读；两个写开关和准确确认短语仍然生效。托管连接保留旧令牌自动更新配置，外部手动连接在 Agent 接入页执行“更新 OpenClaw 配置”命令一次。自动化模型目录直接跟随当前个人 Agent 的 OpenClaw 配置，刷新不调用模型或启动执行器。Inscope 不托管模型密钥，也不会把文章正文预先发送给模型。',
      },
      {
        title: '快捷输入 @ 与 /',
