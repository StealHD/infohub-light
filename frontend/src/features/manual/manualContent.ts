...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '查看与保存',
        description: '卡片可以从标签或空白区域展开，也可用展开/折叠线框按钮控制；鼠标或键盘停留在底部操作上时，说明会优先显示在按钮上方。展开后只显示第一张本地可查看图片作为完整代表缩略图，其余图片仅在放大预览中切换。预览会在电脑和手机的可见区域内完整容纳横图与竖图；多图可用缩略图、左右按钮、方向键或触摸滑动切换，单图不显示多余导航，加载失败可就地重试。也可以打开原文、收藏或“问 Agent”；取消收藏或忽略后有 8 秒撤销入口，超时后仍可从设置页恢复忽略内容。',
      },
      {
        title: '管理收藏',
