  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        description: 'Owner/Admin 可在“设置 → ActorOps”切换 X、Instagram 或 YouTube 抓取类型。标准配置选择两个不同发布者的 Actor 即可启用，第三槽只由管理员主动补充；严格候选不足时可查看放宽项与保留底线，明确选择单路兼容。兼容候选会跨免费搜索保留同一路线里每个 Actor 的最新安全证据，后续空搜索不会把已合格的 1 个候选清掉；Build、Schema、价格或失败指纹变化时仍会重新判定。付费试跑和 1/3 生效仍各自确认，普通新增来源始终不显示 Actor。主备配置默认只显示三槽、人类可读生命周期和唯一下一步；免费搜索进行时显示查询进度并自动刷新，完成后分别显示原始候选数、当前可选择数与失败原因，不再保留可无限连点的主操作。相同 Actor/Build 的失败会被记住；确认 Store 中证据变化后才从候选详情重新检查。若提示“Actor 返回的内容不属于本次目标”，表示结果来自其他账号/频道，或缺少可证明目标归属的字段，可能由推荐内容、默认账号、旧缓存或字段映射造成。来源启用直接列出既有账号或频道，可为单个来源选择自动或首选 Actor；首选旧数据、失败或暂停时仍自动切备。运行与告警可用专用校验 Key 按默认 24 小时、6–168 小时或关闭的频率比较主备，也可单独确认费用后立即校验；页面显示上次、下次、实际费用、单轮和理论月上限。诊断时间线汇总发现、验证、费用、切备、新鲜度、Key 与人工动作，只显示公共 Actor 名称和安全原因码。候选、Build、费用账本、Revision/回滚与 Discovery AI 位于默认收起的高级详情；地址、签名、Chat ID、Token、真实目标、Actor 输入与远端 Run/Dataset 都不会回显。YouTube 始终先用公开 Atom Feed，Actor fallback 一路即可运行且不会自动追逐第三槽。',
        href: '/settings/actorops',
        linkLabel: '打开 ActorOps',
      },
      {
