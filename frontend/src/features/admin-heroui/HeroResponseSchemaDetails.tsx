...
function SchemaTable({ title, schema }: { title: string; schema?: ResponseSchemaSummary | null }) {
  return <section className="min-w-0"><h5 className="text-sm font-medium">{title}</h5>{!schema ? <p className="mt-2 text-sm text-muted">本次运行未记录该结构。</p> : <><p className="mt-1 text-xs text-muted">根类型：{schema.root_type}{schema.truncated ? ' · 已截断' : ''}</p>{schema.fields.length ? <div className="mt-2 max-h-72 overflow-auto"><table className="w-full table-fixed text-left text-xs"><thead><tr className="border-b border-separator"><th className="w-[72%] py-2">字段路径</th><th>类型</th></tr></thead><tbody>{schema.fields.map((field) => <tr key={`${field.path}:${field.type}`} className="border-b border-separator"><td className="overflow-wrap-anywhere py-2 font-mono">{field.path}</td><td>{field.type}</td></tr>)}</tbody></table></div> : <p className="mt-2 text-sm text-muted">没有可展示的字段。</p>}</>}</section>
}
...
export function HeroResponseSchemaDetails({ job, sourceNames }: { job: Job; sourceNames: ReadonlyMap<string, SourceName> }) {
  ...
  return <details className="mt-3"><summary className="cursor-pointer text-xs text-muted">响应结构</summary>{!values.length && <p className="mt-2 text-sm text-muted">本次运行未记录响应结构。</p>}<div className="mt-3 grid gap-4">{values.map((schema, index) => {
    const source = sourceNames.get(schema.source_id)
    const name = typeof source === 'string' ? source : source?.display_name || schema.source_id
    return <section key={`${schema.source_id}:${index}`} className="min-w-0 border-t border-separator pt-3"><div className="flex flex-wrap items-center gap-2"><h4 className="font-medium">{name}</h4>{schema.catalog_type && <Chip size="sm" variant="soft"><Chip.Label>{schema.catalog_type}</Chip.Label></Chip>}{schema.capture_status && <Chip size="sm" variant="soft"><Chip.Label>{labels[schema.capture_status] || '未记录'}</Chip.Label></Chip>}</div>{schema.capture_status && messages[schema.capture_status] && <p className="mt-2 text-sm text-muted">{messages[schema.capture_status]}</p>}{schema.job_truncated && <p className="mt-2 text-sm text-muted">字段较多，已按安全上限截断。</p>}<div className="mt-3 grid min-w-0 gap-4 min-[760px]:grid-cols-2"><SchemaTable title="上游原始结构" schema={schema.upstream} /><SchemaTable title="系统标准化结构" schema={schema.normalized} /></div></section>
  })}</div></details>
}
