...
export function HeroSoftDisclosure({
  ...
  return <div
    ...
    className={`${open ? 'w-full basis-full' : 'w-auto'} min-w-0 rounded-lg transition-colors duration-[var(--inteliscope-motion-disclosure)] ${className}`}
  >
    <button
      type="button"
      aria-expanded={open}
      aria-controls={contentId}
      className={`type-meta inline-flex min-h-7 items-center gap-1.5 rounded-lg px-2 transition-colors duration-[var(--inteliscope-motion-disclosure)] hover:bg-default hover:text-foreground focus-visible:outline-2 focus-visible:outline-focus ${open ? 'bg-default text-foreground' : 'text-muted'}`}
      onClick={() => {
        const next = !open
        setOpen(next)
        onOpenChange?.(next)
    ...
    >
      <Icons.ChevronRight
        size={14}
        aria-hidden="true"
        className={`shrink-0 transition-transform duration-[var(--inteliscope-motion-disclosure)] motion-reduce:transition-none ${open ? 'rotate-90' : ''}`}
      />
      {label}
    </button>
    <div
      id={contentId}
      aria-hidden={!open}
      className={`grid transition-[grid-template-rows] duration-[var(--inteliscope-motion-disclosure)] ease-out motion-reduce:transition-none ${open ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}
    >
