...
export function HeroSubscriptionsPage() {
  ...
  const subscribeMutation = useMutation({ mutationFn: (source: CatalogSource) => api.subscribe(source.id), onMutate: (source) => feedback.begin('subscribe', source.id), onSuccess: async (_result, source) => { await invalidate(); feedback.succeed('subscribe', source.id, `${source.display_name} 订阅成功。`) }, onError: (caught, source) => feedback.fail('subscribe', source.id, mutationError(caught)) })
  const unsubscribeMutation = useMutation({ mutationFn: ({ subscription }: { source: CatalogSource; subscription: Subscription }) => api.unsubscribe(subscription.id), onMutate: ({ source }) => feedback.begin('unsubscribe', source.id), onSuccess: async (_result, { source }) => { await invalidate(); feedback.succeed('unsubscribe', source.id, `${source.display_name} 已取消订阅。`) }, onError: (caught, { source }) => feedback.fail('unsubscribe', source.id, mutationError(caught)) })
  ...
  const localFeedbackKeys = [
    ...
    ...sources.flatMap((source) => [
      { action: 'subscribe', entity: source.id },
      { action: 'unsubscribe', entity: source.id },
