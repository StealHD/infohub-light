...
export function HeroSubscriptionsPage() {
  ...
  return <div className="h-full overflow-y-auto"><div className="mx-auto grid w-full max-w-[1440px] gap-5 p-4 min-[768px]:p-6">
    ...
    {sources.map((source) => { const phase = feedback.phase('source-fetch', source.id); const message = feedback.message('source-fetch', source.id); if (!message || !phase || ['pending', 'queued', 'running'].includes(phase)) return null; return <HeroNotice key={`source-feedback-${source.id}`} title={message} status={phase === 'succeeded' ? 'success' : phase === 'partial' ? 'warning' : 'danger'} role={phase === 'failed' ? 'alert' : 'status'} /> })}
