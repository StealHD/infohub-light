...
export function HeroSubscriptionsPage() {
  ...
  const scheduleMutation = useMutation({ mutationFn: (patch: { enabled: boolean; interval_minutes: number }) => api.updateFeedSchedule(patch), onMutate: () => feedback.begin('feed-schedule', 'global'), onSuccess: () => { feedback.succeed('feed-schedule', 'global', '自动更新设置已保存。'); return queryClient.invalidateQueries({ queryKey: queryKeys.feedSchedule(user.id) }) }, onError: (caught) => feedback.fail('feed-schedule', 'global', mutationError(caught)) })
  const subscribeMutation = useMutation({ mutationFn: (source: CatalogSource) => api.subscribe(source.id), onMutate: (source) => feedback.begin('subscribe', source.id), onSuccess: (_result, source) => { feedback.succeed('subscribe', source.id, `${source.display_name} 订阅成功。`); return invalidate() }, onError: (caught, source) => feedback.fail('subscribe', source.id, mutationError(caught)) })
  const unsubscribeMutation = useMutation({ mutationFn: ({ subscription }: { source: CatalogSource; subscription: Subscription }) => api.unsubscribe(subscription.id), onMutate: ({ source }) => feedback.begin('unsubscribe', source.id), onSuccess: (_result, { source }) => { feedback.succeed('unsubscribe', source.id, `${source.display_name} 已取消订阅。`); return invalidate() }, onError: (caught, { source }) => feedback.fail('unsubscribe', source.id, mutationError(caught)) })
  const retryMutation = useMutation({ mutationFn: (job: Job) => api.retryJob(job.id), onMutate: (job) => feedback.begin('retry-job', job.id), onSuccess: (_result, job) => { feedback.succeed('retry-job', job.id, '重试任务已提交。'); return invalidate() }, onError: (caught, job) => feedback.fail('retry-job', job.id, mutationError(caught)) })
