...
import { sourceActivationDescription } from './sourceActivationToast'
    ...
    <div className="grid min-w-0 gap-3">
      ...
      <div className="grid min-w-0 gap-3 min-[480px]:grid-cols-[minmax(0,1fr)_auto] min-[480px]:items-end">
        <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1">
          ...
          {error && <RefreshButton size="sm" variant="ghost" pending={loading} label="重试" onPress={onRetry} />}
        </div>
        <HeroSelect
          label="更新周期"
...
export function HeroSubscriptionsPage() {
  ...
  const activeSetupType = editingSource ? effectiveSourceType(editingSource) : createType
  const activeDefinition = definitions.find((definition) => definition.type === activeSetupType)
    ?? (editingSource?.type === 'apify_social'
      ? {
          type: 'apify_social',
          label: '旧社交来源',
          availability: 'temporarily_unavailable' as const,
          unavailable_reason: 'platform_setup_pending' as const,
          fields: [],
        }
      : undefined)
  const activeDefinitionUnavailable = activeDefinition?.availability === 'temporarily_unavailable'
  ...
  const subscribeMutation = useMutation({ mutationFn: (source: CatalogSource) => api.subscribe(source.id), onMutate: (source) => feedback.begin('subscribe', source.id), onSuccess: async (result, source) => { await refreshCatalogAndContent(); feedback.clear('subscribe', source.id); const reused = result.subscription.reused_item_count ?? 0; actionToast.success(`${source.display_name} 订阅成功`, { description: sourceActivationDescription(result.source_activation, reused) }) }, onError: (caught, source) => { feedback.clear('subscribe', source.id); actionToast.danger(`${source.display_name} 订阅失败`, { description: mutationError(caught) }) } })
  ...
  const visibleJobs = useMemo(() => (
    (jobsQuery.data?.jobs ?? []).filter((job) => (
      job.user_id === user.id
      && shouldShowJob(job)
      && (isAdmin || !isActorOpsJob(job))
    ))
  ), [isAdmin, jobsQuery.data, user.id])
      ...
      const fetchLabel: SubscriptionViewEntry['fetchLabel'] = phase === 'pending'
        ...
        : activeJob?.status === 'running' || phase === 'running'
          ...
          : activeJob?.status === 'queued' || phase === 'queued'
            ? '已排队'
            : '立即获取'
      const hasSafeEditor = Boolean(definitions.find((definition) => definition.type === effectiveSourceType(entry.source)))
        || entry.source.type === 'apify_social'
      const canEdit = hasSafeEditor && canEditSource(user, entry.source)
      return {
        ...entry,
        fetchLabel,
  ...
  return <div ref={pageScrollerRef} data-page-scroll-region className="quiet-scroll-region h-full min-w-0 overflow-x-hidden overflow-y-auto"><PageFrame width="admin" className="grid min-w-0 gap-5 px-4 pb-4 pt-2 min-[768px]:px-6 min-[768px]:pb-6">
    ...
    <Tabs selectedKey={tab} onSelectionChange={(key) => selectTab(String(key))}>
      ...
      <Tabs.Panel id="subscriptions" className="grid gap-5 pt-5">
        {loading
          ...
          : <SubscriptionListView
              ...
              />}
              onFetch={(entry) => fetchMutation.mutateAsync({ source: entry.source, subscription: entry.subscription })}
              onToggleNotification={(entry, enabled) => notificationMutation.mutateAsync({ source: entry.source, subscription: entry.subscription, enabled })}
      ...
      <Tabs.Panel id="library" className="grid gap-5 pt-5">
        {loading
          ...
          : <SourceLibraryListView
              ...
              onSubscribe={(source) => subscribeMutation.mutateAsync(source)}
              onUnsubscribe={(entry) => {
                if (entry.subscription) return unsubscribeMutation.mutateAsync({ source: entry.source, subscription: entry.subscription })
              }}
              onEditSource={beginEditSource}
              onShare={beginShare}
            />}
      ...
      <Tabs.Panel id="jobs" className="grid gap-3 pt-5">
        <p className="type-body text-muted">{isAdmin ? '这里只展示你需要关心的运行结果；平台内部调度成功记录会自动隐藏。' : '这里只展示与你的订阅直接相关的运行结果。'}</p>
        ...
        {jobsQuery.isError && <HeroNotice title="运行记录读取失败" status="warning">
          ...
          <RefreshButton size="sm" variant="ghost" className="ml-2" pending={jobsQuery.isFetching} label="重试" onPress={() => jobsQuery.refetch()} />
        </HeroNotice>}
        {visibleJobs.map((job) => {
          ...
          return <Card key={job.id} data-compact-job-card variant="secondary" className="min-w-0 max-w-full border border-separator bg-surface-secondary p-3 shadow-none">
            ...
            <div className="mt-2 flex min-w-0 flex-wrap items-center gap-x-4 gap-y-1 border-t border-separator pt-2">
              ...
              {editable && job.retryable && !presented.actorOpsIssue && <StableAsyncButton size="sm" variant="ghost" className="ml-auto" aria-label={retryPending ? `重试中 ${presented.title}` : undefined} pending={retryPending} pendingContent="重试中" onPress={() => retryMutation.mutateAsync(job)}>重试</StableAsyncButton>}
      ...
      sourceDialogNeedsSecret && secretsQuery.isLoading
        ...
        : sourceDialogNeedsSecret && secretsQuery.isError
          ? <HeroNotice title="可用 Key 读取失败" status="warning">
              <RefreshButton size="sm" variant="ghost" className="ml-2" pending={secretsQuery.isFetching} label="重试" onPress={() => secretsQuery.refetch()} />
            </HeroNotice>
          : <div className="grid gap-4">
              {activeDefinitionUnavailable && <HeroNotice title="平台连接字段暂时锁定" status="warning">
                <p>仍可修改来源名称、说明、默认频道和主题；目标与启用状态保持不变。</p>
                {isAdmin && <Button size="sm" variant="ghost" className="mt-2" onPress={() => navigate(unavailableSourceCopy(activeDefinition).settingsHref)}>{unavailableSourceCopy(activeDefinition).settingsLabel}</Button>}
              </HeroNotice>}
              <SourceForm
                ...
                taxonomy={taxonomy}
                submitLabel="保存来源"
                configLocked={activeDefinitionUnavailable}
      ...
      {activeDefinition && (
        activeDefinitionUnavailable
          ? <HeroNotice title={unavailableSourceCopy(activeDefinition).title} status="warning">
              <p>{unavailableSourceCopy(activeDefinition).description}</p>
              {isAdmin && <Button size="sm" variant="ghost" className="mt-2" onPress={() => navigate(unavailableSourceCopy(activeDefinition).settingsHref)}>{unavailableSourceCopy(activeDefinition).settingsLabel}</Button>}
            </HeroNotice>
          : sourceDialogNeedsSecret && secretsQuery.isLoading
          ? <LoadingState label="正在读取可用 Key" rows={2} />
  ...
  <HeroDialog isOpen={Boolean(shareSource)} onOpenChange={(open) => !open && closeShareSource()} returnFocusRef={shareSourceReturnFocus} title={shareSource ? `分享 ${shareSource.display_name}` : '分享来源'}>
    <div className="grid gap-4">
      ...
      <StableAsyncButton pending={shareMutation.isPending} pendingContent="分享中…" onPress={() => shareSource ? shareMutation.mutateAsync({ source: shareSource, scope: 'public' }) : undefined}>确认公开并转交管理权</StableAsyncButton>
    </div>
  </HeroDialog>
  </div>
}
