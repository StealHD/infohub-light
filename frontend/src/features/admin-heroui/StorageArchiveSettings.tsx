  ...
  return <div className="grid gap-7">
    <SettingsSection title="存储概览" description="工作区稳定内容、在线媒体与冷归档的当前占用。">
      {summary.isPending
        ...
        : summary.isError
          ? <HeroNotice title="存储状态读取失败" status="warning"><RefreshButton size="sm" variant="ghost" pending={summary.isFetching} label="重试此区域" onPress={() => summary.refetch()} /></HeroNotice>
          : summary.data && <>
    ...
    <SettingsSection
      ...
      actions={<RefreshButton size="sm" variant="ghost" pending={archives.isFetching} onPress={() => archives.refetch()} />}
    >
