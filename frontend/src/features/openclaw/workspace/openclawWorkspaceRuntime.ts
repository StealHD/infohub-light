...
export function createOpenClawWorkspaceRuntime(refs: OpenClawLifecycleRefs): { controller: OpenClawWorkspaceController; routeEvent(event: GatewayEvent): void } {
  ...
  const controller: OpenClawWorkspaceController = {
    ...
    async listProjects() { return projectWorkspaceProjects(await request('projects.list', { includeObserved: false })) },
    ...
    async listArtifacts(scope) { const trustedScope = requireArtifactScope(scope); return projectArtifacts(await request('artifacts.list', trustedScope), trustedScope) },
    async getArtifact(artifactId, scope) { const id = requireText(artifactId, 'Artifact'); const trustedScope = requireArtifactScope(scope); return projectArtifactDetail(await request('artifacts.get', { artifactId: id, ...trustedScope }), { artifactId: id, ...trustedScope }) },
    async downloadArtifact(artifactId, scope) { const id = requireText(artifactId, 'Artifact'); const trustedScope = requireArtifactScope(scope); return projectArtifactDownload(await request('artifacts.download', { artifactId: id, ...trustedScope }), { artifactId: id, ...trustedScope }) },
    async skillsStatus() {
      const agentId = refs.session.agentId; const sessionKey = refs.session.sessionKey
      const result = projectSkillsStatus(await request('skills.status', { agentId: agentId ?? undefined }))
