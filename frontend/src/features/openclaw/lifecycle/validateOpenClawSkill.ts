...
export async function validateOpenClawSkill(refs: OpenClawLifecycleRefs, snapshot: OpenClawSendSnapshot, gatewayUrl: string) {
  ...
  const status = await client.request('skills.status', { agentId }).then(async (value) => (await import('../workspace/openclawWorkspaceDetails')).projectSkillsStatus(value)).catch(() => { throw new OpenClawSkillValidationError() })
