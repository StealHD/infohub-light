...
import { Chip, Popover, StatusIndicator, useHoverPopoverIntent } from '../../design-system'
import { actorOpsV2CandidateIssueLabel, actorOpsV2CandidateLabel, actorOpsV2CandidateStatusLabel, actorOpsV2PriceLabel, actorOpsV2PublicActorSlug, compactNumber, type ActorOpsV2CandidateView } from './actorOpsV2RouteModel'
...
export function ActorOpsV2ActorChip({ candidate }: { candidate: ActorOpsV2CandidateView | null }) {
  ...
  const status = actorOpsV2CandidateStatusLabel(candidate)
  const issue = actorOpsV2CandidateIssueLabel(candidate)
  return <div className="flex min-w-0 flex-wrap items-center gap-1.5">
    <Popover isOpen={open} onOpenChange={hoverIntent.onOpenChange}>
      <Popover.Trigger
        ref={triggerRef}
      ...
      <Popover.Content isNonModal ref={contentRef} placement="bottom start" offset={8} containerPadding={12} className="z-50 w-[min(340px,calc(100vw-24px))] p-0" {...hoverIntent.surfaceProps}>
        <Popover.Dialog aria-label={`${name} 商城信息`} className="grid gap-3 p-4">
          ...
          {status && <div className="grid gap-1 rounded-lg border border-separator bg-surface-secondary p-3"><StatusIndicator label={status} tone={candidate.operational_status === 'confirmed_failure' ? 'danger' : 'warning'} /><p className="type-meta text-muted">{issue}{candidate.last_failure_at ? ` · ${formatFailureTime(candidate.last_failure_at)}` : ''}</p>{candidate.retry_at && <p className="type-meta text-muted">最早重试：{formatFailureTime(candidate.retry_at)}</p>}<p className="type-meta text-muted">{candidate.operational_status === 'confirmed_failure' ? '系统已停止调度这个 Actor，可由管理员手动替换。' : '系统会在退避结束后半开探测；再次成功后提示会自动清除。'}</p></div>}
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2 type-meta">
            ...
            <Metric label="头像映射" value={avatarMappingLabel(candidate.avatar_mapping_status)} />
          </dl>
    ...
    {status && <StatusIndicator label={status} tone={candidate.operational_status === 'confirmed_failure' ? 'danger' : 'warning'} />}
  </div>
}
...
function avatarMappingLabel(status: ActorOpsV2CandidateView['avatar_mapping_status']) {
  return { ready: '已就绪', missing: '待发现', stale: '待刷新' }[status]
}
