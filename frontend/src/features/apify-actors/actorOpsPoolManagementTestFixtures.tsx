...
export function poolManagementDetail(): ApifyActorRouteDetail {
  ...
  return {
    ...
    slots: [
      ...
      { slot: 'backup_1', revision_id: backup.revision_id, runnable: true, revision: backup, actions: { add: false, replace: true, remove: true } },
...
export function renderPoolManagement(selected = poolManagementDetail(), overrides: Partial<ServiceApi> = {}) {
  ...
  const api = {
    ...
    refreshApifyActorPoolCandidates: vi.fn(), removeApifyActorRouteActivePoolSlot: vi.fn().mockResolvedValue(selected), ...overrides,
