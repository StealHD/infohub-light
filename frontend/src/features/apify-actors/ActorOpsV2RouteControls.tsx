...
export function ActorOpsV2RouteControls({ route }: { route: ActorOpsV2RouteView }) {
  ...
  return <div className="flex items-center gap-1.5">
    ...
    <Popover isOpen={moreOpen} onOpenChange={setMoreOpen}>
      ...
      <Popover.Content placement="bottom end" offset={6} containerPadding={8} className="z-50 w-44 p-0">
        <Popover.Dialog aria-label="Actor 路由更多操作" className="grid gap-0.5 p-2">
          ...
          {route.binding_summary.pending_count > 0 && <Button size="sm" variant="ghost" className="justify-start" isDisabled={verify.isPending} onPress={() => { setMoreOpen(false); setVerifyOpen(true) }}>核验待处理来源</Button>}
        </Popover.Dialog>
      </Popover.Content>
    </Popover>
    ...
    <ConfirmDialog open={verifyOpen} heading="核验待处理来源" description="系统只检查已保存的来源证据、目标指纹和当前主备顺序。不会抓取或启动 Actor。" confirmation="确认核验来源绑定" pending={verify.isPending} onClose={() => setVerifyOpen(false)} onConfirm={() => verify.mutate()} />
...
function PriceCapControl({ route, onSaved }: { route: ActorOpsV2RouteView; onSaved: () => Promise<unknown> }) {
  ...
  return <Popover isOpen={open} onOpenChange={(next) => { setOpen(next); if (next) { setValue(route.per_run_cap_usd.toFixed(2)); setConfirmation('') } }}>
    <Popover.Trigger<'button'> type="button" className="rounded-lg px-2 py-1 type-meta text-muted outline-none hover:bg-surface-secondary hover:text-foreground focus-visible:outline-2 focus-visible:outline-focus">费用</Popover.Trigger>
...
function ConfirmDialog({ target, open, heading, description, confirmation, pending, onClose, onConfirm }: {
  target?: ActorOpsV2CandidateView | null; open?: boolean; heading?: string; description?: string
  confirmation?: '确认设为主用 Actor' | '确认核验来源绑定'; pending: boolean; onClose: () => void; onConfirm: () => void
}) {
  const [value, setValue] = useState('')
  const visible = Boolean(target || open)
  const expected = confirmation || '确认设为主用 Actor'
  const title = heading || '设为当前主用'
  const detail = description || `将 ${actorOpsV2CandidateLabel(target || null)} 设为主用；不会启动 Actor，也不会产生费用。`
...
function actionError(error: unknown, fallback: string) {
  ...
  const labels: Record<string, string> = { actorops_v2_candidate_switch_conflict: '路线已更新，请刷新后再切换。', actorops_v2_binding_conflict: '路线已更新，请刷新后重新核验。', actorops_v2_binding_evidence_missing: '现有证据还不足以安全启用来源；没有执行抓取。', actorops_v2_price_cap_confirmation_required: '提高费用上限需要输入确认短语。', actorops_v2_unavailable: 'ActorOps v2 当前不可用。' }
