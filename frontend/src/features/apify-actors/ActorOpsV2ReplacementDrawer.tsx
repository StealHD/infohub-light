...
function CandidateSelection({ targets, selectedTarget, systemCandidates, staticCandidates, sampleCandidates, blockedCandidates, mappingCandidates, incompleteCount, loading, selected, cap, total, discovery, busy, onTarget, onSelect, onPreview, onDiscover, onMetadata }: {
  ...
  return <div className="grid gap-3">
    ...
    <div className="flex flex-wrap gap-2"><StableAsyncButton variant="secondary" pending={discoveryActive} pendingContent="正在搜索候选…" isDisabled={busy} onPress={onDiscover}>搜索更多候选</StableAsyncButton><StableAsyncButton variant="ghost" pending={false} pendingContent="更新中…" isDisabled={busy} onPress={onMetadata}>更新商城信息</StableAsyncButton></div>
    ...
    <StableAsyncButton pending={busy} pendingContent="处理中…" isDisabled={!selectedTarget || !selected || total <= 0} onPress={onPreview}>{selectedTarget ? '免费检查并准备实测' : '先选择替换位置'}</StableAsyncButton>
  </div>
}
