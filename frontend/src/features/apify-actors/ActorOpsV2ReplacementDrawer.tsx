...
import { actionToast, Button, Drawer, StableAsyncButton, StatusNotice } from '../../design-system'
...
function CandidateSelection({ targets, selectedTarget, systemCandidates, staticCandidates, sampleCandidates, blockedCandidates, mappingCandidates, incompleteCount, loading, selected, cap, total, discovery, busy, previewPending, discoveryPending, metadataPending, onTarget, onSelect, onPreview, onDiscover, onMetadata }: {
  ...
  onTarget: (target: ActorOpsV2ReplacementTarget) => void; onSelect: (candidate: ActorOpsV2CandidateView) => void; onPreview: () => Promise<unknown>; onDiscover: () => Promise<unknown>; onMetadata: () => Promise<unknown>
}) {
  ...
  return <div className="grid gap-3">
    ...
    <StableAsyncButton pending={previewPending} pendingContent="处理中…" isDisabled={(busy && !previewPending) || !selectedTarget || !selected || total <= 0} onPress={onPreview}>{selectedTarget ? '免费检查并准备实测' : '先选择替换位置'}</StableAsyncButton>
  </div>
}
...
function PlanStep({ plan, slotLabel, busy, authorizePending, applyPending, cancelPending, revalidatePending, onAuthorize, onApply, onCancel, onRevalidate, onReset }: {
  ...
  if (plan.status === 'ready') return <div className="grid gap-3">{progress}<p className="type-control">全部 {plan.binding_count} 条来源已通过，将把 {actorOpsV2CandidateLabel(plan.candidate)} 应用到{slotLabel}。</p><div className="flex justify-end gap-2"><StableAsyncButton variant="ghost" isDisabled={busy && !cancelPending} pending={cancelPending} pendingContent="取消中…" onPress={onCancel}>取消计划</StableAsyncButton><StableAsyncButton isDisabled={busy && !applyPending} pending={applyPending} pendingContent="应用中…" onPress={onApply}>应用到{slotLabel}</StableAsyncButton></div></div>
  if (plan.status === 'previewed') return <div className="grid gap-3">{progress}<p className="type-control">免费预检已通过。实测将按来源串行进行，每个来源最多 ${plan.per_probe_cap_usd.toFixed(2)}，总额不超过 ${plan.total_cap_usd.toFixed(2)}。</p><div className="flex justify-end gap-2"><StableAsyncButton variant="ghost" isDisabled={busy && !cancelPending} pending={cancelPending} pendingContent="取消中…" onPress={onCancel}>取消计划</StableAsyncButton><StableAsyncButton isDisabled={busy && !authorizePending} pending={authorizePending} pendingContent="启动中…" onPress={onAuthorize}>开始实测，最高 ${plan.total_cap_usd.toFixed(2)}</StableAsyncButton></div></div>
  if (plan.status === 'authorized' || plan.status === 'running') return <div className="grid gap-3">{progress}
    ...
    <p className="type-meta text-muted">取消只会停止计划继续执行；已发起或待对账的运行、Dataset 与费用事实仍会保留。</p><StableAsyncButton variant="ghost" isDisabled={busy && !cancelPending} pending={cancelPending} pendingContent="取消中…" onPress={onCancel}>取消替换计划</StableAsyncButton></div>
  if (plan.status === 'failed') return <div className="grid gap-3">{progress}<StatusNotice title="替换未完成" status="danger"><span>{replacementError(plan.error_code)}</span><span className="mt-1 block">{replacementFailureCostMessage(plan.error_code)}</span></StatusNotice>{REVALIDATABLE_ERRORS.has(plan.error_code || '') && <StatusNotice title="可以零费用重验" status="info">系统会只读原已结算 Dataset，并用当前字段规则重新核验；不会启动 Actor，也不会改写原费用记录。</StatusNotice>}<div className="flex flex-wrap gap-2">{REVALIDATABLE_ERRORS.has(plan.error_code || '') && <StableAsyncButton isDisabled={busy && !revalidatePending} pending={revalidatePending} pendingContent="重验中…" onPress={onRevalidate}>重新验证已有结果（$0 Actor 费）</StableAsyncButton>}<Button variant="secondary" isDisabled={busy} onPress={onReset}>重新选择候选</Button></div></div>
