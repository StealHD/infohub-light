...
function configurationFor(mcpUrl: string): string {
...
export function AgentsPage() {
  ...
  const configuration = useMemo(
    () => configurationFor(query.data?.mcp_url || '<MCP_URL>'),
    [query.data?.mcp_url],
  )
  ...
  return <Box sx={{ height: '100%', overflow: 'auto', p: { xs: 2, md: 3 }, maxWidth: 1200, mx: 'auto' }}>
    <Stack spacing={3}>
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ alignItems: { sm: 'center' }, justifyContent: 'space-between' }}>
        <Box><Typography component="h1" variant="h4" sx={{ fontWeight: 800 }}>助手连接</Typography><Typography color="text.secondary">让你自己电脑上的 OpenClaw 只读访问当前账户的数据。</Typography></Box>
        <Stack direction="row" spacing={1}>
          ...
          <Button variant="contained" startIcon={<SmartToyRounded />} disabled={creationDisabled} onClick={() => setCreateOpen(true)}>创建连接</Button>
        </Stack>
      </Stack>
    ...
    <Dialog open={createOpen} onClose={() => !createPending && setCreateOpen(false)} fullWidth maxWidth="sm" aria-labelledby="create-agent-title"><DialogTitle id="create-agent-title">创建助手连接</DialogTitle><DialogContent><TextField autoFocus fullWidth margin="dense" label="连接名称" value={createName} onChange={(event) => setCreateName(event.target.value)} slotProps={{ htmlInput: { maxLength: 80 } }} helperText={`令牌有效 ${query.data.token_ttl_days} 天，只会显示一次。`} /></DialogContent><DialogActions><Button onClick={() => setCreateOpen(false)} disabled={createPending}>取消</Button><Button variant="contained" disabled={!createName.trim() || createPending} onClick={() => void createConnection()}>{createPending ? '生成中…' : '生成一次性令牌'}</Button></DialogActions></Dialog>

    <Dialog open={Boolean(oneTimeToken)} onClose={() => undefined} fullWidth maxWidth="md" aria-labelledby="one-time-token-title"><DialogTitle id="one-time-token-title">保存一次性令牌</DialogTitle><DialogContent><Alert severity="warning" sx={{ mb: 2 }}>关闭后无法恢复。请先保存到本机环境文件，再明确确认。</Alert><Stack spacing={2}><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ alignItems: { sm: 'center' } }}><Box component="code" sx={{ flex: 1, p: 1.5, bgcolor: 'surfaceContainer', borderRadius: 1, overflowWrap: 'anywhere' }}>{oneTimeToken}</Box><Button startIcon={<ContentCopyRounded />} onClick={() => oneTimeToken && void copy(oneTimeToken, '令牌已复制。')}>复制令牌</Button></Stack><Box><Typography variant="subtitle2" gutterBottom>1. 写入本机环境文件并限制权限</Typography><Box component="pre" aria-label="本地令牌环境命令" tabIndex={0} sx={{ p: 1.5, bgcolor: 'surfaceContainer', borderRadius: 1, overflow: 'auto', whiteSpace: 'pre-wrap' }}>{'INTELISCOPE_MCP_TOKEN=<一次性令牌>\nchmod 0600 ~/.openclaw/.env'}</Box></Box><Box><Typography variant="subtitle2" gutterBottom>2. 配置并检查 OpenClaw</Typography><Box component="pre" data-testid="openclaw-config" aria-label="OpenClaw 配置命令" tabIndex={0} sx={{ p: 1.5, bgcolor: 'surfaceContainer', borderRadius: 1, overflow: 'auto', whiteSpace: 'pre-wrap' }}>{configuration}</Box></Box></Stack></DialogContent><Divider /><DialogActions><Button variant="contained" onClick={() => { setOneTimeToken(null); setNotice('一次性令牌已从页面清除。') }}>我已保存</Button></DialogActions></Dialog>
