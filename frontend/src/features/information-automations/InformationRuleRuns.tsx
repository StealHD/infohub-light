...
export function InformationRuleRuns({ ruleId }: { ruleId: string }) {
  ...
  return <section className="grid gap-3" aria-label="提醒运行记录">
    ...
    {query.data?.pages.flatMap((page) => page.items).map((run) => <Card key={run.id} className="p-4" variant="secondary">
      ...
      {run.range && <p className="type-meta">本批 {run.range.item_count} 条 · {run.model?.id}</p>}
      ...
      {run.result && <><p className="type-body">{run.result.summary}</p><p className="type-body">{run.result.reason}</p>
        {run.result.evidence.map((item, index) => <blockquote key={index} className="type-body">{item.quote} — {item.note}{item.url && <a className="block underline" href={item.url} target="_blank" rel="noopener noreferrer">{item.title || '查看原文'}</a>}</blockquote>)}</>}
