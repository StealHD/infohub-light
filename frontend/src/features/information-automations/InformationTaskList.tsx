...
export function InformationTaskList({ rules, selected, onSelect, loading, hasMore, testStatuses = {}, canMutate = false, action = null, dirtyRuleId, onTransition, onDelete }: {
  ...
  canMutate?: boolean; action?: InformationRuleActionState
  dirtyRuleId?: string | null; onDelete?: (rule: InformationRule) => Promise<void>
  ...
  return <div className="grid min-w-0 gap-4">
    ...
    <ul aria-label="自动化任务" className="grid min-w-0 gap-2">
      {visible.map((rule) => <li key={rule.id} className="min-w-0"><Card variant="secondary" className="p-0">
        <div className={`flex min-w-0 items-center gap-2 rounded-[var(--inteliscope-radius-card)] ${selected === rule.id ? 'bg-default' : 'hover:bg-default'}`}>
        <button id={`information-task-${rule.id}`} type="button" aria-label={`查看与编辑 ${rule.config.name}`} aria-expanded={selected === rule.id}
          onClick={() => onSelect(rule.id)} className="flex min-w-0 flex-1 items-center gap-3 rounded-[var(--inteliscope-radius-card)] p-4 text-left focus-visible:outline focus-visible:outline-accent">
          ...
          <span className="min-w-0 flex-1"><span className="type-control block break-words [overflow-wrap:anywhere]">{rule.config.name}</span>
            <span className="type-meta mt-1 block truncate text-muted">{ruleStateLabels[rule.state]} · {triggerLabel(rule.config.trigger)}
        ...
        {(onTransition || onDelete) && <div className="mr-3 flex shrink-0 items-center gap-1">
          {onTransition && <InformationRuleActivation rule={rule} canMutate={canMutate} dirty={dirtyRuleId === rule.id} action={action} onTransition={onTransition} compact />}
          {onDelete && <InformationRuleDelete rule={rule} canMutate={canMutate} dirty={dirtyRuleId === rule.id} action={action} onDelete={onDelete} compact />}
        </div>}
        </div>
      </Card></li>)}
    </ul>
