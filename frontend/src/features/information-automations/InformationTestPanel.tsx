...
export function InformationTestPanel({ rule, dirty, canMutate, session, onSelect, onStart }: {
  ...
  const pending = ['submitting', 'pending', 'judging', 'quota_wait'].includes(session.phase)
  ...
  return <div className="grid gap-4">
    ...
    <Card variant="secondary" className="grid gap-3 p-4">
      ...
      <StableAsyncButton pending={session.phase === 'submitting'} pendingContent="正在提交…"
        isDisabled={!canMutate || rule.state === 'archived' || dirty || !session.selection.length || pending} onPress={onStart}>
        {session.phase === 'submission_unknown' || session.phase === 'failed' ? '手动重新测试' : '开始测试'}
      </StableAsyncButton>
    </Card>
