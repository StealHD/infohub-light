...
describe('settingsNavigation', () => {
  it('exposes the requested groups and scopes developer settings to administrators', () => {
    ...
    expect(ownerItems.map((item) => item.label)).toEqual(['概览', '来源', '获取与主题', '已忽略内容', 'AI', '通知', '外观', '密钥', '高级'])
  ...
  it('keeps old hashes compatible without exposing administrator pages to members', () => {
    ...
    expect(settingsDestinationFromLegacyHash('#settings-actorops', 'owner')).toBe('/settings/legacy#settings-actorops')
