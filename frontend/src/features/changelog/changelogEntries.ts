...
import {openclawRoleSessionEntry} from './openclawRoleSessionEntry';import {sourceIdentityRecoveryEntry} from './sourceIdentityRecoveryEntry';import {instagramMediaEntry} from './instagramMediaEntry'
...
import {openclawServerEntries} from './openclawServerEntry';import {stableAsyncButtonChangelogEntry} from './stableAsyncButtonChangelogEntry';import {agentWorkspaceChangelogEntry} from './agentWorkspaceChangelogEntry';import {automationUiPolishEntry} from './automationUiPolishEntry';import {telegramTopicsOpenclawNotificationEntry} from './telegramTopicsOpenclawNotificationEntry';import {releaseReadinessEntry} from './releaseReadinessEntry'
export const changelogMonths:import('./changelogTypes').ChangelogMonth[]=[{id:'month-2026-09',label:'2026 年 9 月',entries:[instagramMediaEntry,releaseReadinessEntry,telegramTopicsOpenclawNotificationEntry,automationUiPolishEntry,openclawRoleSessionEntry,sourceIdentityRecoveryEntry,openclawModelRecoveryEntry,automationRecoveryEntry,automationLayoutEntry,composerPanelEntry,automationsUnifiedEntry,...openclawServerEntries,agentWorkspaceChangelogEntry,stableAsyncButtonChangelogEntry]}, {
...
codeHealthMaintenanceEntry,pageHeaderChangelogEntry,subscriptionCommandBarChangelogEntry,
      ...
      },
      {
        ...
        items: [
          ...
          { title: '旧帖子不再冒充成功', description: '系统按每个来源的发布时间和稳定内容指纹维护水位；同一内容的重复 Dataset 行会合并，结果始终按最新发布时间排序后才应用每次获取数量。有新内容才增加成功计数，只有已见内容保持中性，结果倒退会立即切到备用。同一 Actor 连续三次只返回已见内容后，下一次正常周期会改用一个健康备用；当次不会额外启动付费 Actor，备用推进水位后成为该来源的后续首选。' },
