    ...
    entries: [
      {
        ...
        items: [
          ...
          { title: '内部稳定性持续收敛', description: '新增按现有旧债渐进收紧的代码规模门禁，并在服务关闭和测试结束时可靠释放数据库连接；同时移除未挂载的 ActorOps、通知旧表单、不再被现行页面调用的客户端 helper 与重复请求封装，将系统健康、认证及存储治理 HTTP 路由和 Worker 启动迁移检查收口到独立模块，把 ActorOps 的纯展示映射和确认对话框移出状态控制器，并解除 Store 与 Feed repository 的运行时反向导入。登录、Settings 外壳与 OpenClaw 对话视图改为按需分包，首屏 JavaScript 门禁收紧到 240 KiB；OpenClaw 后台运行与重连状态继续常驻。前端未使用局部变量和参数现在由 TypeScript 直接阻断，Fast Refresh 从 10 条旧警告清零并提升为错误门禁。后端兼容端点继续保留，现行页面、HTTP/OpenAPI、迁移顺序、审批权限、付费动作、历史书签、任务与数据语义保持不变。' },
        ],
      },
      {
        date: '2026-08-11',
