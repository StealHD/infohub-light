    ...
    entries: [
      {
        date: '2026-08-03',
        title: '设置成为独立工作区',
        summary: '设置现在使用类似系统设置的独立侧栏与内容区，不再占用或改变信息流主侧栏；首批迁移概览、外观和通知。',
        items: [
          { title: '返回原来的应用位置', description: '进入设置后左侧完整切换为设置导航，顶部“返回应用”会恢复打开前的页面、查询条件与定位；直接打开设置深链时安全回到信息流。' },
          { title: '设置导航按职责分组', description: '概览、来源、AI、通知、外观与高级按工作区、智能、通信、系统和开发者分组；高级仅对 Owner/Admin 显示，本阶段不增加设置搜索。' },
          { title: '三个页面率先原生迁移', description: '概览以统一卡片呈现角色和入口，外观继续写入原有明暗偏好，通知完整复用既有服务与个人通知组件；没有新增 API、数据库或业务写入。' },
          { title: '手机使用全高设置侧栏', description: '小于 768 px 时内容全屏显示，顶部按钮打开与桌面相同的角色过滤 Drawer；选择页面后自动关闭，并保持无横向溢出。' },
          { title: '旧链接继续可用', description: '旧关于和通知 hash 会转到新页面；AI、已忽略内容、获取与主题、存储归档和密钥继续由兼容设置页承载，原有权限、草稿与保存逻辑不变。' },
        ],
      },
      {
        date: '2026-08-03',
