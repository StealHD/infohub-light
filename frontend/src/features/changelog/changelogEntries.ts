// Reviewed for source-bound healthy-slot execution, certified-candidate visibility/counts, direct YouTube Actor runtime with observed content contracts, and release health; entries remain grouped by user-visible release date.
    ...
    entries: [
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'YouTube 只走认证 Actor', description: '搜索中显示查询轮次并自动刷新，不能重复提交；终态分别显示原始候选数、当前可选择数和失败原因。YouTube 候选不足时明确保持等待认证，频道地址仅做身份，不会再由 Atom/RSS 回退伪造可运行；重新检查只用于已确认的 Actor/Build 证据变化。' },
