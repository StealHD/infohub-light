    ...
    entries: [
      {
        ...
        items: [
          ...
          { title: '内部稳定性持续收敛', description: '新增按现有旧债渐进收紧的代码规模门禁，并在服务关闭和测试结束时可靠释放数据库连接；同时移除未挂载的 ActorOps、通知旧表单、不再被现行页面调用的客户端 helper 与重复请求封装，并将系统健康、认证路由和 Worker 启动迁移检查收口到独立模块。后端兼容端点继续保留，现行页面、HTTP/OpenAPI、迁移顺序、历史书签、任务与数据语义保持不变。' },
        ],
      },
      {
        date: '2026-08-11',
