...
export const agentWorkspaceChangelogEntry: ChangelogEntry = {
  ...
  items: [
    ...
    { title: '信息流 Agent 统一模型选择', description: '信息流右侧 Agent 与完整 OpenClaw 对话共用模型和思考胶囊浮层，不再弹出两套不同的下拉选择界面。' },
    { title: '会话与执行反馈更清楚', description: '全部会话和新对话改为标题行右侧的图标操作，去掉会话区与工作区之间的装饰线；自己的消息在完整工作台和信息流 Agent 中统一靠右显示，OpenClaw 回复之间不再使用横线分隔。运行或发送错误与居中的消息轨道对齐，不再贴到页面左缘。展开执行反馈可逐项查看操作、状态与耗时。删除确认可在成功删除时选择下次不再提醒，并可从全部会话随时恢复。' },
