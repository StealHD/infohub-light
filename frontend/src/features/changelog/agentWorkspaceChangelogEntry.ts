...
export const agentWorkspaceChangelogEntry: ChangelogEntry = {
  ...
  items: [
    ...
    { title: '历史会话与紧凑对话界面', description: '侧栏最多显示三条单行会话，“全部会话”支持搜索、分页和归档筛选，可打开当前 Gateway 授权的旧会话。新对话首条消息后由 Gateway 自动总结标题；输入框增加顶部留白并稍微加高，移除聊天滚动区与输入框之间的外部间隔，右侧胶囊打开模型与思考浮层，可用分档滑杆调节并恢复模型默认档位（不列出关和自动），Fast 开启时模型名前显示闪电标志，浮层闪电仅鼠标悬停时显示底色，简短用量提示显示在按钮上方，发送按钮沿用主题色，输入框在未聚焦和聚焦时都没有外沿描边、焦点圈或黑色间隙，以主题表面和光标提示输入位置；闪电可以独立切换 Fast，并用于下一条消息和原失败重试；Fast 显示快速定向粒子与加速用量提醒，Ultra 单独显示轻微漂浮粒子并短暂提醒更多 Token 消耗；两者同时开启以 Fast 动效为主。浮层、字体和图标进一步缩小，白色滑块去黑边并支持悬停微放大，刻度与滑块圆心精确对齐，两端刻度内收至圆角中心，模型名称完整显示，底部模型、思考和用量集中靠右，颜色跟随当前主题；连接表单按钮等宽对齐，窄屏纵向排列；减少动态效果时静止，背景沿用项目主题；检查器柔和开合，长会话名称与箭头独立排版；OpenClaw 左栏与信息流同宽，工作区菜单更紧凑；在 Skills 等页面点会话会正确返回对话；切换期间导航不再上下跳动，选中已有会话不改变列表顺序。账户菜单与信息流统一，使用示例移入独立工作区页面。' },
