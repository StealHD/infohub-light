...
describe('HeroChangelogPage', () => {
  ...
  it('renders source-controlled Chinese entries as an accessible timeline with responsive month navigation', () => {
    ...
    expect(within(latestEntries[0] as HTMLElement).getByText('登录名可以维护')).toBeVisible()
    expect(within(latestEntries[0] as HTMLElement).getByText('删除需要精确确认')).toBeVisible()
    expect(within(latestEntries[0] as HTMLElement).getByText('运行与私人数据安全收尾')).toBeVisible()
    ...
    expect(within(latestEntries[1] as HTMLElement).getByText('来源级总结与追问')).toBeVisible()
