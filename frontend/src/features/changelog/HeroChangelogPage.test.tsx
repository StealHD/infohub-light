...
describe('HeroChangelogPage', () => {
  ...
  it('renders source-controlled Chinese entries as an accessible timeline with responsive month navigation', () => {
    ...
    expect(screen.getByRole('heading', { level: 3, name: '通知目标统一配置并由多个业务复用' })).toBeInTheDocument()
    ...
    expect(within(entries[0] as HTMLElement).getByText('一个目的地只维护一次')).toBeVisible()
    expect(within(entries[0] as HTMLElement).getByText('升级保留配置与历史')).toBeVisible()
    ...
    expect(within(entries[2] as HTMLElement).getByText('一个编号串联整次故障')).toBeVisible()
    expect(within(entries[2] as HTMLElement).getByText('后续开发自动受检')).toBeVisible()
    expect(within(entries[2] as HTMLElement).getByText('2026-07-30')).toHaveAttribute('datetime', '2026-07-30')
    expect(within(entries[3] as HTMLElement).getByText('零条目也能保留头像')).toBeVisible()
    expect(within(entries[3] as HTMLElement).getByText('B 站与免费来源有安全回退')).toBeVisible()
    expect(within(entries[3] as HTMLElement).getByText('2026-07-30')).toHaveAttribute('datetime', '2026-07-30')
    expect(within(entries[4] as HTMLElement).getByText('七个分区统一生效')).toBeVisible()
    expect(within(entries[4] as HTMLElement).getByText('按需读取仍保留')).toBeVisible()
    expect(within(entries[4] as HTMLElement).getByText('2026-07-30')).toHaveAttribute('datetime', '2026-07-30')
    expect(within(entries[5] as HTMLElement).getByText('七类配置统一')).toBeVisible()
    expect(within(entries[5] as HTMLElement).getByText('平台业务响应可验证')).toBeVisible()
    expect(within(entries[5] as HTMLElement).getByText('2026-07-30')).toHaveAttribute('datetime', '2026-07-30')
    expect(within(entries[6] as HTMLElement).getByText('修复机器人消息协议')).toBeVisible()
    expect(within(entries[6] as HTMLElement).getByText('通用 Webhook 保持兼容')).toBeVisible()
    expect(within(entries[6] as HTMLElement).getByText('2026-07-29')).toHaveAttribute('datetime', '2026-07-29')
    expect(within(entries[7] as HTMLElement).getByText('占位内容不会进入信息流')).toBeVisible()
    expect(within(entries[7] as HTMLElement).getByText('费用有硬保护')).toBeVisible()
    expect(within(entries[7] as HTMLElement).getByText('2026-07-29')).toHaveAttribute('datetime', '2026-07-29')
    expect(within(entries[8] as HTMLElement).getByText('最终页才提示')).toBeVisible()
    expect(within(entries[8] as HTMLElement).getByText('AI 后台低优先级生成')).toBeVisible()
    expect(within(entries[8] as HTMLElement).getByText('2026-07-29')).toHaveAttribute('datetime', '2026-07-29')
    expect(within(entries[9] as HTMLElement).getByText('低频页面按需加载')).toBeVisible()
    expect(within(entries[9] as HTMLElement).getByText('稳定数据减少重复请求')).toBeVisible()
    expect(within(entries[9] as HTMLElement).getByText('2026-07-29')).toHaveAttribute('datetime', '2026-07-29')
    expect(within(entries[10] as HTMLElement).getByText('通用解析入口')).toBeVisible()
    expect(within(entries[10] as HTMLElement).getByText('名称由 Agent 发现')).toBeVisible()
    expect(within(entries[10] as HTMLElement).getByText('2026-07-29')).toHaveAttribute('datetime', '2026-07-29')
    expect(within(entries[11] as HTMLElement).getByText('默认跟随全局')).toBeVisible()
    expect(within(entries[11] as HTMLElement).getByText('卡片信息更准确')).toBeVisible()
    expect(within(entries[11] as HTMLElement).getByText('2026-07-28')).toHaveAttribute('datetime', '2026-07-28')
  })
