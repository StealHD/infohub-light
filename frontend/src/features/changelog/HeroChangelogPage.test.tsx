...
describe('HeroChangelogPage', () => {
  ...
  it('renders source-controlled Chinese entries as an accessible timeline with responsive month navigation', () => {
    ...
    expect(within(entries[0] as HTMLElement).getByText('低频页面按需加载')).toBeVisible()
    expect(within(entries[0] as HTMLElement).getByText('稳定数据减少重复请求')).toBeVisible()
    expect(within(entries[0] as HTMLElement).getByText('2026-07-29')).toHaveAttribute('datetime', '2026-07-29')
    expect(within(entries[1] as HTMLElement).getByText('通用解析入口')).toBeVisible()
    expect(within(entries[1] as HTMLElement).getByText('名称由 Agent 发现')).toBeVisible()
    expect(within(entries[1] as HTMLElement).getByText('2026-07-29')).toHaveAttribute('datetime', '2026-07-29')
    expect(within(entries[2] as HTMLElement).getByText('默认跟随全局')).toBeVisible()
    expect(within(entries[2] as HTMLElement).getByText('卡片信息更准确')).toBeVisible()
    expect(within(entries[2] as HTMLElement).getByText('2026-07-28')).toHaveAttribute('datetime', '2026-07-28')
  })
