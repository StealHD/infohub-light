...
describe('HeroChangelogPage', () => {
  ...
  it('renders source-controlled Chinese entries as an accessible timeline with responsive month navigation', () => {
    ...
    expect(latestEntries).toHaveLength(30)
    ...
    expect(within(latestTimeline).getByText('X 只升级当前三路')).toBeVisible()
    expect(within(latestTimeline).getByText('今日与近 N 天直达信息流')).toBeVisible()
    expect(within(latestTimeline).getByText('历史入口保持不变')).toBeVisible()
    expect(within(latestTimeline).getByText('React 成为唯一界面')).toBeVisible()
    expect(within(latestTimeline).getByText('历史数据原样保留')).toBeVisible()
    expect(within(latestTimeline).getByText('新内容提示保持可点击')).toBeVisible()
    expect(within(latestTimeline).getByText('登录名可以维护')).toBeVisible()
    expect(within(latestTimeline).getByText('当前 Actor 始终可见')).toBeVisible()
    expect(within(latestTimeline).getByText('超时可直接调整')).toBeVisible()
    expect(within(latestTimeline).getByText('池成员由管理员决定')).toBeVisible()
    expect(within(latestTimeline).getByText('按来源连续快速阅读')).toBeVisible()
    expect(within(latestTimeline).getByText('每次只有一个下一步')).toBeVisible()
    expect(within(latestTimeline).getByText('保留一层输入边框')).toBeVisible()
    expect(within(latestTimeline).getByText('精确复用 main 绿灯')).toBeVisible()
    expect(within(latestTimeline).getByText('图片只在浏览器规范化')).toBeVisible()
    expect(within(latestTimeline).getByText('筛选取代侧边导航')).toBeVisible()
    expect(within(latestTimeline).getByText('状态下方直接看条数')).toBeVisible()
    expect(within(latestTimeline).getByText('每条附件对齐右侧')).toBeVisible()
    expect(within(latestTimeline).getByText('原网页读取透明降级')).toBeVisible()
    expect(within(latestTimeline).getByText('AI 设置更紧凑')).toBeVisible()
    expect(within(latestTimeline).getByText('只读取固定公开页面')).toBeVisible()
    expect(within(latestTimeline).getByText('帮助与版本整行可点')).toBeVisible()
    expect(within(latestTimeline).getByText('触底文案不再强绑工作区 AI Key')).toBeVisible()
    expect(within(latestTimeline).getByText('设置侧栏与主侧栏同规格')).toBeVisible()
    expect(within(latestTimeline).getByText('设置路由验证更稳定')).toBeVisible()
    expect(within(latestTimeline).getByText('全部设置统一在独立工作区')).toBeVisible()
    expect(within(latestTimeline).getByText('存储与归档有了独立入口')).toBeVisible()
    expect(within(latestTimeline).getByText('独立路由与旧链接兼容')).toBeVisible()
    expect(within(latestTimeline).getByText('工作区导航更完整')).toBeVisible()
    expect(within(latestTimeline).getByText('密钥页不再打开旧设置')).toBeVisible()
    expect(within(latestTimeline).getByText('返回原来的应用位置')).toBeVisible()
    expect(within(latestTimeline).getByText('频道 ID 不再传错')).toBeVisible()
  })
