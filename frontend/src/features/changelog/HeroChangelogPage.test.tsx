...
describe('HeroChangelogPage', () => {
  ...
  it('renders source-controlled Chinese entries as an accessible timeline with responsive month navigation', () => {
    ...
    expect(latestEntries).toHaveLength(6)
    expect(latestEntries[0]).toHaveAttribute('aria-current', 'true')
    expect(within(latestEntries[0] as HTMLElement).getByText('存储与归档有了独立入口')).toBeVisible()
    expect(within(latestEntries[1] as HTMLElement).getByText('独立路由与旧链接兼容')).toBeVisible()
    expect(within(latestEntries[2] as HTMLElement).getByText('工作区导航更完整')).toBeVisible()
    expect(within(latestEntries[3] as HTMLElement).getByText('密钥页不再打开旧设置')).toBeVisible()
    expect(within(latestEntries[4] as HTMLElement).getByText('返回原来的应用位置')).toBeVisible()
    expect(within(latestEntries[5] as HTMLElement).getByText('频道 ID 不再传错')).toBeVisible()
  })
