...
describe('HeroChangelogPage', () => {
  ...
  it('renders source-controlled Chinese entries as an accessible timeline with responsive month navigation', () => {
    ...
    expect(latestEntries).toHaveLength(12)
    expect(latestEntries[0]).toHaveAttribute('aria-current', 'true')
    expect(within(latestEntries[0] as HTMLElement).getByText('只读取固定公开页面')).toBeVisible()
    expect(within(latestEntries[1] as HTMLElement).getByText('帮助与版本整行可点')).toBeVisible()
    expect(within(latestEntries[2] as HTMLElement).getByText('触底文案不再强绑工作区 AI Key')).toBeVisible()
    expect(within(latestEntries[3] as HTMLElement).getByText('设置侧栏与主侧栏同规格')).toBeVisible()
    expect(within(latestEntries[4] as HTMLElement).getByText('设置路由验证更稳定')).toBeVisible()
    expect(within(latestEntries[5] as HTMLElement).getByText('全部设置统一在独立工作区')).toBeVisible()
    expect(within(latestEntries[6] as HTMLElement).getByText('存储与归档有了独立入口')).toBeVisible()
    expect(within(latestEntries[7] as HTMLElement).getByText('独立路由与旧链接兼容')).toBeVisible()
    expect(within(latestEntries[8] as HTMLElement).getByText('工作区导航更完整')).toBeVisible()
    expect(within(latestEntries[9] as HTMLElement).getByText('密钥页不再打开旧设置')).toBeVisible()
    expect(within(latestEntries[10] as HTMLElement).getByText('返回原来的应用位置')).toBeVisible()
    expect(within(latestEntries[11] as HTMLElement).getByText('频道 ID 不再传错')).toBeVisible()
  })
