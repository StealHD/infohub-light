...
export const codeHealthMaintenanceEntry = {
  ...
  items: [
    ...
    { title: '连接与模块边界更可靠', description: '服务关闭和测试结束会可靠释放数据库连接；系统认证、成员管理、Feed 读取与媒体授权、Feed/来源周期、存储治理、Jobs 与 Agent 连接 HTTP 适配、Worker 的迁移、路由恢复、周期入队、来源与 Feed handler、Actor 校验与新鲜度 handler、通知 backlog、claim-token 事务终结以及 commit 后通知/终态遥测、Store/Feed 边界以及 ActorOps 展示和确认逻辑拆到更小的内部模块。未挂载的旧表单、无调用 helper 和重复请求封装已删除，后端兼容端点继续保留。' },
