...
export function AgentWorkspacePage() {
  ...
  const workspaceHeader = <AgentWorkspaceHeader
    chat={chat}
  ...
  const content = location.pathname === '/agent/examples'
    ...
    : location.pathname === '/agent/automations'
      ? new URLSearchParams(location.search).get('advanced') === 'cron' ? <AgentAutomationsView chat={chat} /> : <Suspense fallback={<p role="status">正在加载提醒…</p>}><InformationAutomationsView key={user.id} canMutate={user.role !== 'viewer'} header={workspaceHeader} /></Suspense>
  ...
  return <div className="h-full min-h-0 min-w-0 overflow-hidden">
    ...
    >
      {location.pathname !== '/agent/automations' || new URLSearchParams(location.search).get('advanced') === 'cron' ? workspaceHeader : null}
