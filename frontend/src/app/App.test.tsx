...
describe('App routes', () => {
  ...
  it('switches Feed to source overview without another Feed request and restores it after a global search', async () => {
    ...
    try {
      ...
      const summaryCacheKey = 'inteliscope.source-summary.v3:user-live'
      ...
      window.localStorage.removeItem('inteliscope.source-summary.v3:user-live')
  ...
  it('explains a live single-source fetch block in an overlay without queueing work', async () => {
    ...
    const message = await screen.findByText('后台 Worker 心跳已过期，本次任务未创建，也未产生抓取费用。恢复 Worker 后再试。')
    expect(message.closest('[data-slot="toast-region"]')).toHaveTextContent('阻塞来源 获取未开始')
  ...
  it('shows successful Key creation once in a top overlay without adding a page notice', async () => {
    ...
    await browser.type(screen.getByRole('textbox', { name: '独立 Base URL（可选）' }), 'https://gateway.example.test/deepseek')
  ...
  it('manages one ordered Apify pool without exposing or mutating an active key', async () => {
    ...
    const api = liveApi({
      ...
      secrets: vi.fn().mockResolvedValue({ secrets: [
        ...
        { id: 'apify-backup-two', name: 'APIFY_BACKUP_TWO', kind: 'apify', provider: 'apify', env_name: 'APIFY_BACKUP_TWO', is_set: true, used_by: [] },
    ...
    expect(screen.queryByText(/当前无运行中任务/)).not.toBeInTheDocument()
    ...
    await waitFor(() => expect(screen.queryByRole('dialog', { name: '轮换 APIFY_BACKUP_TWO' })).not.toBeInTheDocument())
  ...
  it('searches AI Keys and derives the model default from the selected Key', async () => {
    const browser = userEvent.setup()
    ...
    expect(screen.getByText('选择用于分析的 Key 和模型。')).toBeInTheDocument()
    expect(screen.queryByRole('textbox', { name: 'Base URL' })).not.toBeInTheDocument()
    const aiKey = await screen.findByRole('combobox', { name: 'AI Key' })
    await browser.click(aiKey)
    await browser.type(aiKey, 'deepseek')
    await browser.click(await screen.findByRole('option', { name: /^DeepSeek Key/ }))
    expect(screen.getByRole('textbox', { name: '模型' })).toHaveValue('deepseek-v4-flash')
    expect(aiKey).toHaveValue('DeepSeek Key')
    expect(screen.getAllByText('deepseek · 默认地址 · 已设置').length).toBeGreaterThan(0)
    expect(screen.queryByText('Provider')).not.toBeInTheDocument()
    expect(screen.queryByText('连接地址随 Key 独立保存；这里不会显示真实 Key 值。')).not.toBeInTheDocument()
  ...
  it('saves feed-end copy settings and exposes generation status with a queued refresh', async () => {
    ...
    expect(screen.queryByRole('button', { name: '展开空列表完整文案列表' })).not.toBeInTheDocument()
    const statusDisclosure = screen.getByRole('button', { name: /AI 文案可用/ })
    expect(statusDisclosure).toHaveAttribute('aria-expanded', 'false')
    await browser.click(statusDisclosure)
    ...
    await waitFor(() => expect(screen.queryByText('有尚未保存的更改')).not.toBeInTheDocument())
    expect(screen.getByRole('combobox', { name: '生成用 AI Key' })).toHaveValue('OpenAI Copy Key')
