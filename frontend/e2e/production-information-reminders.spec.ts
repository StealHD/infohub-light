...
for (const colorMode of ['light', 'dark'] as const) test(`personal reminder draft, direct start and test preview stay separate (${colorMode})`, async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: '编辑任务：研究提醒', exact: true }).click()
  ...
  const savedDetails = testInfo.project.name === 'desktop'
    ? page.getByRole('complementary', { name: '自动化任务详情' })
    : page.getByRole('dialog', { name: '未保存草稿', exact: true })
  const launch = savedDetails.getByRole('button', { name: '启动任务：未保存草稿', exact: true })
  await expect(launch).toBeEnabled()
  await launch.focus()
  await page.keyboard.press('Enter')
  await expect(savedDetails.getByRole('button', { name: '暂停任务：未保存草稿', exact: true })).toBeVisible()
...
test('task list search, selection, pending close and focus preserve the editor', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: '恢复任务：旧研究任务', exact: true }).dblclick()
  ...
  const openedDetails = testInfo.project.name === 'desktop'
    ? page.getByRole('complementary', { name: '自动化任务详情' })
    : page.getByRole('dialog', { name: '每日研究简报', exact: true })
  const editTask = openedDetails.getByRole('button', { name: '编辑任务：每日研究简报', exact: true })
  await expect(editTask).toBeVisible()
  await openedDetails.getByRole('button', { name: '展开完整描述' }).hover()
  await expect(page.getByRole('tooltip')).toContainText('展开完整描述')
  ...
  if (testInfo.project.name === 'desktop') {
    ...
    const header = page.locator('header[data-page-header-appearance="inset"]')
    const headerBox = (await header.boundingBox())!
    const panelBox = (await panel.boundingBox())!
    expect(Math.abs(headerBox.x + headerBox.width - panelBox.x)).toBeLessThanOrEqual(8)
