...
test('usage examples are reachable on every viewport and never dispatch work', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('[data-agent-examples]').getByRole('heading', { level: 3 })).toHaveCount(7)
  await expect(page.locator('[data-agent-examples]').getByRole('heading', { name: /Worktree/i })).toHaveCount(0)
