...
test('Changelog entry points expose the responsive month navigation', async ({ page }, testInfo) => {
  ...
  await expect(currentDetails.getByRole('listitem')).toHaveCount(6)
  await expect(currentDetails.getByText('占位内容不会进入信息流')).toBeVisible()
  await expect(currentDetails.getByText('三路串行主备')).toBeVisible()
  await expect(currentDetails.getByText('恢复不额外花钱')).toBeVisible()
  await expect(currentDetails.getByText('费用有硬保护')).toBeVisible()
  await expect(currentDetails.getByText('状态与告警集中查看')).toBeVisible()
  await expect(currentDetails.getByText('付费试跑必须逐次确认')).toBeVisible()
