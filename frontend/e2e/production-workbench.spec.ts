...
test('Changelog entry points expose the responsive month navigation', async ({ page }, testInfo) => {
  ...
  await expect(currentDetails.getByText('零条目也能保留头像')).toBeVisible()
  await expect(currentDetails.getByText('B 站与免费来源有安全回退')).toBeVisible()
  await expect(currentDetails.getByText('所有入口显示当前头像')).toBeVisible()
