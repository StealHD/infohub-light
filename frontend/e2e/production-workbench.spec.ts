...
test('Changelog entry points expose the responsive month navigation', async ({ page }, testInfo) => {
  ...
  await expect(currentDetails.getByText('七个分区统一生效')).toBeVisible()
  await expect(currentDetails.getByText('折叠页面也能继续')).toBeVisible()
  await expect(currentDetails.getByText('按需读取仍保留')).toBeVisible()
