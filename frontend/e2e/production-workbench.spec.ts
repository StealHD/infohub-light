...
async function expectLocatorInside(inner: Locator, outer: Locator) {
  await expect.poll(async () => {
    ...
    return innerBounds.x >= outerBounds.x - 1
      && innerBounds.y >= outerBounds.y - 1
      && innerBounds.x + innerBounds.width <= outerBounds.x + outerBounds.width + 1
...
test.beforeEach(async ({ page }) => {
  await installProductionWorkbenchApiMocks(page, {
    items, rollingItem, batchRollingItems, savedRouteItem, historyRouteItem, tsuchaHistoryItems, socialRouteItem,
  })
})
...
test('Changelog entry points expose the responsive month navigation', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('heading', { name: '故障排查可以从请求串到后台结果' })).toBeVisible()
  await expect(page.getByRole('heading', { name: '来源头像不再依赖新内容' })).toBeVisible()
  await expect(page.getByRole('heading', { name: '设置分区随滚动自然加载' })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Webhook 通知适配七类常用接收端' })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'X 抓取可在三路 Actor 间安全切换' })).toBeVisible()
  ...
  const latestTimeline = page.getByRole('list', { name: '2026 年 8 月更新记录' })
  ...
  const latestEntry = latestTimeline.locator(':scope > li').first()
  const firstEntry = entries.first()
  const previousEntry = entries.nth(1)
  const observabilityEntry = entries.filter({ hasText: '故障排查可以从请求串到后台结果' })
  await expect(latestTimeline).toBeVisible()
  await expect(timeline).toBeVisible()
  await expect(latestEntry).toHaveAttribute('aria-current', 'true')
  await expect(firstEntry).not.toHaveAttribute('aria-current')
  await expect(previousEntry).not.toHaveAttribute('aria-current')
  await expect(observabilityEntry.locator('time[datetime="2026-07-30"]')).toBeVisible()
  const currentDetails = observabilityEntry.getByRole('list')
  await expect(currentDetails.getByRole('listitem')).toHaveCount(4)
  await expect(currentDetails.getByText('一个编号串联整次故障')).toBeVisible()
  await expect(currentDetails.getByText('日志写入失败可被发现')).toBeVisible()
  await expect(currentDetails.getByText('工作区诊断必须显式授权')).toBeVisible()
  await expect(currentDetails.getByText('后续开发自动受检')).toBeVisible()
  await expect(entries.last().locator('[data-timeline-connector]')).toBeHidden()
  const firstConnectorBox = await firstEntry.locator('[data-timeline-connector]').boundingBox()
...
test('the production theme defaults to night and persists explicit day and night choices', async ({ page }, testInfo) => {
  ...
  const timeline = page.getByRole('list', { name: '2026 年 8 月更新记录' })
