...
`src/services/apify_actor_ops.py::ApifyActorOpsService` 是所有 `platform + target_type + capability` Route Profile、不可变 Adapter Revision、固定三槽、来源绑定、Canary 证据、CAS generation 与发布栅栏的唯一所有者；`x/profile` 只是永久兼容 route key。每条 Route 精确为 `primary/backup_1/backup_2`，前两槽必须 `certified`，第三槽只可为 `probationary|certified`，Actor 必须各不相同且至少两个发布者；运行准入少于两个 runnable 时 fail closed。候选认证完成后的生产排槽也由该服务确定性计算；浏览器只提交 Route generation 与精确启用确认，不得指定推荐 Revision，服务在同一写事务中重算 2+1 方案并调用既有完整激活校验。一次逻辑任务冻结 Route/binding/Key generation、三个 Revision、Actor ID、精确 Build 和 Manifest hash，串行调用并在写共享缓存或 Feed 前重新校验；旧 generation 可以完成远端结算但不得污染新配置。
