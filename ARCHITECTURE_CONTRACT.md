...
`src/services/apify_actor_ops.py::ApifyActorOpsService` 是所有 `platform + target_type + capability` Route Profile、不可变 Adapter Revision、固定三槽、来源绑定、Canary 证据、CAS generation 与发布栅栏的唯一所有者；`x/profile` 只是永久兼容 route key。服务端优先生成 `certified/certified/probationary-or-certified` 的完整 2+1；若暂不可达，则可生成两个各有成功 Canary、Actor 与发布者均不同、固定 Build 的 probationary/certified 快速池，第三槽保持 NULL。运行准入始终少于两个 runnable 时 fail closed，两个 runnable 时为 degraded，第三槽后续以 generation 热补位。浏览器只提交 Route generation 与精确启用确认，不得指定推荐 Revision，服务在同一写事务中重算方案并执行完整校验。一次逻辑任务冻结 Route/binding/Key generation、当前实际运行的两或三个 Revision、Actor ID、精确 Build 和 Manifest hash，串行调用并在写共享缓存或 Feed 前重新校验；旧 generation 可以完成远端结算但不得污染新配置。
...
`src/services/apify_actor_canary.py` 只执行已由 `ApifyActorOpsService` 原子批准并入队的 one-shot Canary；Route 认证优先由 `apify_actor_canary_batches` 协调一次管理员确认下最多三个服务端候选，来源级验证继续逐槽确认。每个管理员动作携带唯一 `approval_id`，服务端只保存其摘要、批准 generation、plan hash 与费用上限；同一动作的网络重放必须返回原 batch/validation/job，任何参数漂移都冲突。Worker 严格串行，并在每个付费 POST 前通过 `ApifyClient.preflight_actor_revision` 免费读取公开 Actor 和精确 Build；确定性不存在、私有、不可运行或 Build 漂移必须以 `$0` 停用 Revision且不创建 attempt。两个不同 Actor/发布者成功后立即把未启动项以 `$0` 终结；候选耗尽仍不足两路时保留成功证据并自动排入一次只做 Store/AI proposal 的 Discovery 补位。未知启动结果阻断整批且不得继续。Canary timeout 缺省 300 秒并在 180–900 秒内按下一 Job 热加载；超时只中止已知远端 Run，不自动重试。远端终态费用由 `apify_actor_runs` 向 attempt/validation/batch 幂等对账，批准上限与实际已终结费用始终分开；确认明文、approval ID、validation ID、真实 target 与远端标识不得进入公共 API、Job 结果或日志。
...
ActorOps feature schema v15 依赖 v13/v14，已有数据库普通初始化不得静默安装。本地共享 migration ledger 的 15/16 已由通知 schema 占用，因此 ActorOps 使用全局 version 17、仍保留 `apify_actor_ops_v15` 名称与脚本；Token 实测扩展使用 `apify_discovery_limits_v16`、global version 18；批次审批与费用证据扩展使用 `apify_actor_canary_batches_v17`、global version 19。三次迁移都必须停 API/Worker并跨过 heartbeat 安全窗，以 SQLite backup API 创建 `0600` 备份，离线安装并检查 integrity/foreign keys；不联网、不调用 AI/Actor。global 19 只把 Run/attempt 已证明的实际终态费用标记为 final；无 remote Run/Dataset 且零预留的确定性 start rejection 修复为 `$0` 并停用失效 Revision，不能证明未启动的记录保持未知，失败必须恢复备份。迁移把现有 X candidate/attempt/费用/健康原样保留，无法证明精确 Build 的适配器只生成 `legacy_builtin` Revision；旧 Discovery Run 的 Token 字段保持 NULL。首期 Route tuple 仅为 X profile、YouTube channel 与 Instagram profile；支持目录和两个支持检查入口共用同一白名单。日常 Actor、Build、Manifest、槽位顺序和 Discovery 调用边界按 generation 从 SQLite 热加载；Discovery Job 开始时只冻结全局 AI 当前 provider/model/base URL、管理员首选 Key 与生产输出上限，不建立独立 AI 配置或 Key 回退池。AI 调用固定单次、180 秒，无自动重试；只有管理员确认的容量测试临时使用 32K/64K，且仅持久化安全 usage/finish/耗时/验证指标。

### 3.7 Secret Boundary
Service DB 和 catalog 只保存环境变量名或 secret ref 元数据，不保存真实密钥、Webhook URL 或 Webhook 签名 Secret。真实 AI/Apify 值与 write-only Webhook URL/可选签名值由 `src/services/secret_store.py` 独占写入 Git/Docker 忽略的 `data/secrets.env`，多值变更必须先构造完整新状态再原子替换且权限为 `0600`；SQLite 只保存用途绑定的确定性变量名、SHA-256 摘要和非秘密 Provider 元数据。API/Worker 可以热加载该文件，但 API、日志、job、Feed、outbox、DOM 和非管理员 source 投影不得返回真实值。Apify pool 表只引用 `secret_id/version` 和安全状态；活动、排空中或仍有非终态 Run 的成员不得轮换或删除，必须先走安全排空。`source_catalog.secret_env` 在池模式只保留回滚兼容，不参与读取、展示或新来源写入。
...
`src/services/preferred_source_notifications.py::PreferredSourceNotificationService` 独占当前用户邮箱/Webhook 目的地设置、新文章差集判定、schema v9 outbox、用户级模拟测试和提交后调度；schema v14 扩展 Provider/签名元数据并由公共 API 投影为 settings schema v2。ServiceStore 只保存 write-safe setting metadata、订阅 opt-in/启用时间与不可回退 generation、Webhook URL/签名值的一致性摘要和 delivery 状态；两类真实值只存在于 `SecretStore` 的不同用户专属环境变量，不能进入 SQLite、config JSON、Feed、Job、日志或 API 响应。设置 partial PATCH 必须在 SQLite 写锁内重读实时用户并合并，任何 SecretStore mutation 前再次确认 enabled/writable role；摘要与用户专属变量不匹配时配置、staging 和发送全部 fail closed，显式清空负责删除确定性变量下的 orphan 值。Provider、URL 或签名变化必须推进 generation 并清除测试状态，generation-guarded 测试结果不能覆盖并发新配置。
