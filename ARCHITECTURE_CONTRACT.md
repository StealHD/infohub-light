...
ActorOps feature schema v15 依赖 v13/v14，已有数据库普通初始化不得静默安装。本地共享 migration ledger 的 15/16 已由通知 schema 占用，因此 ActorOps 使用全局 version 17、仍保留 `apify_actor_ops_v15` 名称与脚本；Token 实测扩展使用独立 `apify_discovery_limits_v16`、global version 18。两次迁移都必须停 API/Worker并跨过 heartbeat 安全窗，以 SQLite backup API 创建 `0600` 备份，离线安装并检查 integrity/foreign keys；不联网、不调用 AI/Actor。迁移把现有 X candidate/attempt/费用/健康原样保留，无法证明精确 Build 的适配器只生成 `legacy_builtin` Revision；旧 Discovery Run 的 Token 字段保持 NULL。首期 Route tuple 仅为 X profile、YouTube channel 与 Instagram profile；支持目录和两个支持检查入口共用同一白名单。日常 Actor、Build、Manifest、槽位顺序和 Discovery 调用边界按 generation 从 SQLite 热加载；Discovery Job 开始时只冻结全局 AI 当前 provider/model/base URL、管理员首选 Key 与生产输出上限，不建立独立 AI 配置或 Key 回退池。AI 调用固定单次、180 秒，无自动重试；只有管理员确认的容量测试临时使用 32K/64K，且仅持久化安全 usage/finish/耗时/验证指标。
