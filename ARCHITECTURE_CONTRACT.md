...
`src/services/apify_actor_canary.py` 只执行已由 `ApifyActorOpsService` 原子批准并入队的 one-shot Canary。每个管理员动作携带唯一 `approval_id`，服务端只保存其摘要、批准 generation 与费用上限；同一动作的网络重放必须返回原 validation/job，任何参数漂移都冲突。确认明文、approval ID、真实 target 与远端标识不得进入公共 API、Job 结果或日志。
