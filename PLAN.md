...
8. **Phase 7 — 实施中：API/UI 收敛**：现有 `/api/admin/apify-*` 路径先作兼容 facade，新增健康、Active/Standby/LKG、degraded reason、无目标 binding 摘要与维护策略投影；默认关闭的 Settings v2 视图不再直接展示 Batch、Stage、多层 generation 或内部 ID。Owner/Admin 可以无费用、CAS 方式把已验证备用设为主用，或对 pending 来源执行已保存证据核验；两者都不启动 Actor。候选 Probe/activate/disable 控制仍待 Phase 6 真实切流验收和独立授权后实现。
