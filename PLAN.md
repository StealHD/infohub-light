...
2026-09-14 发布流程调整：采用[最小发布路径](docs/dev/test-gate.md#发布)，不等待 CI、不收集测试回执或跑 UI 验收。UI skill 在设计/编码时指导遵守既有规则，不管理验收。覆盖下文历史任务中的网页验收、main Gate、Tag smoke 等发布前置要求；功能开发与显式迁移约定保留。本次只修改本地 main 的流程，不执行部署。

2026-09-10 当前任务（优先于以下旧阶段）：从已发布 v2.6.15 / `53afb15c` 修复 VPS OpenClaw 项目链路：个人 MCP 原生协议、模型切换与发送一致性、安全错误反馈及托管独立分析生命周期。沿用现有 Gateway、绑定、账号和凭据；显式 global 44 只增加安装记录，不自动配置历史绑定。先定向测试、差异审查和一次 impacted preflight，再合入 main、精确 CI、tag/Release、本地 amd64 构建及 VPS/适配器部署。上线首先仅同步目录，核对旧积压后才开放领取。用户已授权通过项目个人 Agent 顺序验证 Flash、Pro 和一篇文章分析，单项失败停止后续模型调用，不发送通知。必须区分受控测试、原生协议探测、真实 Gateway 会话工具和真实模型验收；未通过关键项目链路不得宣称完成。
