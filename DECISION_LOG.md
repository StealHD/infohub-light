...
- 兼容/边界：旧 `workspace` 来源仍归公共，`problem` 仍只匹配 `degraded/failing`；来源库频道、历史链接、来源卡状态统计、通知、编辑、分享、立即获取、URL 页签参数和 Service 数据结构均不变。本决策取代 D125 中固定订阅视图与页签行搜索位置的结论；D125 的卡片统计布局继续有效。
...
- 兼容/边界：仅复刻外观与滚动交互，不引入或复制 HeroUI Pro 包或源码；现有 Feed `ViewBar`、筛选、权限、共享搜索、页面 URL、来源库频道、卡片和 Service 合同均不改变。D127 取代 D126 中“页面说明下方的固定半透明工具栏”的视觉结论。

### D128 来源库取消频道导航并收紧工具栏顶部态
...
- 兼容/边界：来源频道字段、来源类型/范围筛选、共享搜索、订阅/取消订阅、编辑、分享、URL 页签参数与 Service 合同均不改变。D128 取代 D126、D127 中来源库继续按频道浏览的结论。
